...
EFFECT.Models = {}
EFFECT.Models[4] = Model("models/shells/fhell_762x51.mdl")
...
function EFFECT:Init(data)
	...
	if not IsValid(data:GetEntity()) then 
		self.Entity:SetModel("models/shells/fhell_762x51.mdl")
		self.RemoveMe = true
		return 
	end
	
	local bullettype = math.Clamp((data:GetScale() or 4), 4, 4)
	...
	self.Entity:SetModel(self.Models[ bullettype ] or "models/shells/fhell_762x51.mdl")
	...
	if IsValid(phys) then
		...
		phys:SetVelocity(direction * math.random(300, 650))
		phys:AddAngleVelocity((VectorRand() * 650))
