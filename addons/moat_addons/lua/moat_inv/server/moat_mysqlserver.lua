...
hook.Add("SQLConnected", "MINVENTORY_MYSQL", function(db)
    MINVENTORY_MYSQL = db
    SQL = include "sql_mysqloo.lua" (db)
...
local function backwards_compatibility__newindex(self, str)
    local real_data = self.__real_data
    print(str, debug.traceback())
end

function m_LoadInventoryForPlayer(ply, cb)
    local backwards_compatibility = {}
    function backwards_compatibility:__newindex(str)
    ply:LoadInventory(function(inv)
        local inv_tbl = {}
        inv_tbl.credits = 0
        local items = {}
        ...
        table.sort(items, function(a, b) return a.uid > b.uid end)
        for i = 1, items do
            inv_tbl["slot_"..i] = items[i]
        end
