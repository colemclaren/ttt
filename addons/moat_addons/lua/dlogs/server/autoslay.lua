util.AddNetworkString("dlogs.SlayMessage")
util.AddNetworkString("dlogs.AutoSlay")
util.AddNetworkString("dlogs.AutoslaysLeft")
...
hook.Add("PlayerAuthed", "dlogsNames", function(ply, steamid)
	for k,v in ipairs(player.GetHumans()) do
		if v == ply then continue end
		net.Start("dlogs.AutoslaysLeft")
		net.WriteEntity(v)
...
function dlogs:GetName(steamid)
	for k,v in ipairs(player.GetHumans()) do
...
function dlogs.SlayMessage(ply, message)
	net.Start("dlogs.SlayMessage")
	net.WriteString(message)
	net.Send(ply)
end

function dlogs:CreateSlayList(tbl)
	if #tbl == 1 then
...
function dlogs:SetSlays(admin, steamid, slays, reason, target)
	if reason == "" then
		reason = dlogs.Autoslay_DefaultReason
	end
	if slays == 0 then
...
hook.Add("TTTBeginRound", "dlogs_AutoSlay", function()
	for k,v in ipairs(player.GetHumans()) do
		if v:IsActive() then
			...
			if data then
				...
				local list = dlogs:CreateSlayList(admins)
				net.Start("dlogs.AutoSlay")
				net.WriteEntity(v)
				net.WriteString(list)
				net.WriteString(reason)
				net.WriteString(dlogs:FormatTime(tonumber(os.time()) - tonumber(_time)))
				net.Broadcast()
