...
function dlogs:SetLineMenu(item, infos, tbl, roles, text, old_logs)
		...
		if item.DamageInfos then
			menu:AddOption(TTTLogTranslate(GetDMGLogLang, "ShowDamageInfos"), function()
				if item.old_logs then
					...
					net.Start("dlogs.AskDamageInfos")
...
function dlogs:AddLogsLine(listview, tbl, roles, nofilters, old)
...
function dlogs:SetListViewTable(listview, tbl, nofilters, old)
...
function dlogs:SetRolesListView(listview, tbl)
	listview:Clear()
...
function dlogs:AddRoleLine(listview, nick, role)
	if (role != 5) then
...
function dlogs:SetDamageInfosLV(listview, roles, att, victim, beg, t, result)
		...
		for k,v in ipairs(nums) do
			...
			for s,i in pairs(info) do
				...
				elseif i[2] == "crowbarpouss" then -- Currently unused
					...
					wep = dlogs:GetWeaponName(i[2]) or TTTLogTranslate(GetDMGLogLang, "UnknownWeapon")
