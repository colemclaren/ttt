surface.CreateFont("dlogs.RDM_Manager", {
    font = "DermaLarge",
    size = 20
})

surface.CreateFont("dlogs.Conclusion", {
    font = "DermaLarge",
    size = 18,
    weight = 600
})

surface.CreateFont("dlogs.ConclusionText", {
    font = "DermaLarge",
    size = 18
})

surface.CreateFont("dlogs.ResponseDisabled", {
    font = "DermaLarge",
    size = 16
})
...
-- lmao string multiline function that supports only 2 lines
-- who makes these jesus fuck
...
cvars.AddChangeCallback("rdm_manager_show_finished", function(name, old, new)
    if IsValid(dlogs.CurrentReports) then
        dlogs.CurrentReports:UpdateAllReports()
    end

    if IsValid(dlogs.PreviousReports) then
        dlogs.PreviousReports:UpdateAllReports()
    end
end)
...
local function TakeAction()
    local report = dlogs.SelectedReport
        ...
        if not report.chat_open then
            menuPanel:AddOption(report.chat_opened and TTTLogTranslate(GetDMGLogLang, "ViewChat") or TTTLogTranslate(GetDMGLogLang, "OpenChat"), function()
                if not report.chat_opened then
                    net.Start("dlogs.StartChat")
                    net.WriteUInt(report.index, 32)
                    net.SendToServer()

                    if not report.response then
                        dlogs.DisableResponse(true)
                    end

                    if report.status == RDM_MANAGER_WAITING then
                        net.Start("dlogs.UpdateStatus")
                        net.WriteUInt(report.previous and 1 or 0, 1)
                        net.WriteUInt(report.index, 16)
    ...
    menuPanel:AddOption(TTTLogTranslate(GetDMGLogLang, "ShowDeathScene"), function()
        local found = false
        local roles = dlogs.Roles[report.round]
        ...
        for k, v in pairs(report.logs or {}) do
            if IsValid(dlogs.events[v.id]) and dlogs.events[v.id].type == "KILL" then
                local infos = v.infos
                local ent = dlogs:InfoFromID(roles, infos[1])
                local att = dlogs:InfoFromID(roles, infos[2])

                if ent.steamid64 == victimID and att.steamid64 == attackerID then
                    net.Start("dlogs.AskDeathScene")
    ...
    local function SetConclusion(ply, num, reason)
        ...
        net.WriteUInt(report.previous and 1 or 0, 1)
        net.WriteUInt(report.index, 16)
        net.WriteString(string.format(TTTLogTranslate(GetDMGLogLang, "AutoReasonSlay"), ply, num, reason))
        net.SendToServer()
    end
    ...
    slaynr:AddOption(TTTLogTranslate(GetDMGLogLang, "ReportedPlayer") .. " (" .. report.attacker_nick .. ")", function()
        local frame = vgui.Create("RDM_Manager_Slay_Reason", dlogs.Menu)
    ...
    slaynr:AddOption(TTTLogTranslate(GetDMGLogLang, "ReportedPlayer") .. " (" .. report.attacker_nick .. ")", function()
        local frame = vgui.Create("RDM_Manager_Ban_Reason", dlogs.Menu)
    ...
    slaynr:AddOption(TTTLogTranslate(GetDMGLogLang, "ReportedPlayer") .. " (" .. report.attacker_nick .. ")", function()
        if IsValid(attacker) then
            RunConsoleCommand("mga", "removeslays", report.attacker)
        else
            RunConsoleCommand("mga", "aslayid", report.attacker, "0", "removing autoslays")
        end
    end):SetImage("icon16/user_delete.png")

    slaynr:AddOption(TTTLogTranslate(GetDMGLogLang, "TheVictim") .. " (" .. report.victim_nick .. ")", function()
        if IsValid(victim) then
            RunConsoleCommand("mga", "removeslays", report.victim)
        else
            RunConsoleCommand("mga", "aslayid", report.victim, "0", "removing autoslays")
        end
    end):SetImage("icon16/user.png")
...
function PANEL:UpdateReport(index)
	...
	if (not IsValid(self.Reports[index])) then
		if (report.status == RDM_MANAGER_FINISHED and not show_finished:GetBool()) then return false end
		...
		self.Reports[index].OnRightClick = function()
			TakeAction()
		end
	else
		self.Reports[index].status = report.status
		...
		if report.conclusion then
			local selected = dlogs.SelectedReport
...
function PANEL:UpdateAllReports()
	...
	if (not dlogs.SelectedReport) then return end
	local selected_current = not dlogs.SelectedReport.Previous
	local current = not self.Previous

	if dlogs.SelectedReport.status != RDM_MANAGER_FINISHED or show_finished:GetBool() then
		for k, v in pairs(self.Lines) do
			v:SetSelected(false)
		end

		dlogs.SelectedReport = nil
		dlogs:UpdateReportTexts()
	elseif selected_current == current then
		for k, v in pairs(self.Lines) do
			if dlogs.SelectedReport.index == v.index then
				v:SetSelected(true)
				break
			end
		end
	...
	if (dlogs.SelectedReport) then
		local report = dlogs.SelectedReport
		local conclusion = report.conclusion

		self.Conclusion:SetText(conclusion and conclusion or TTTLogTranslate(GetDMGLogLang, "NoConclusion"))
		dlogs.DisableResponse((not report.response and report.chat_opened))
	end
...
function PANEL:OnRowSelected(index, line)
	dlogs.SelectedReport = self.ReportsTable[line.index]
	dlogs:UpdateReportTexts()
	local report = dlogs.SelectedReport
	dlogs.DisableResponse((not report.response and report.chat_opened))

	local conclusion = dlogs.SelectedReport.conclusion
	self.Conclusion:SetText(conclusion and conclusion or TTTLogTranslate(GetDMGLogLang, "NoConclusion"))

	if (dlogs.SelectedReport.previous) then
		if (IsValid(dlogs.CurrentReports:GetSelected()[1])) then dlogs.CurrentReports:GetSelected()[1]:SetSelected(false) end
		return
	end

	if (IsValid(dlogs.PreviousReports:GetSelected()[1])) then dlogs.PreviousReports:GetSelected()[1]:SetSelected(false) end
end

vgui.Register("RDM_Manager_ListView", PANEL, "DListView")

net.Receive("dlogs.NewReport", function()
	local tbl = net.ReadTable()
	local index = table.insert(dlogs.Reports.Current, tbl)
	dlogs.Reports.Current[index].index = index

	if IsValid(dlogs.CurrentReports) then
		dlogs.CurrentReports:AddReport(index)
	end
end)

net.Receive("dlogs.UpdateReport", function()
	...
	if previous then
		dlogs.Reports.Previous[index] = updated

		if IsValid(dlogs.PreviousReports) then
			dlogs.PreviousReports:UpdateReport(index)
		end
	else
		dlogs.Reports.Current[index] = updated

		if IsValid(dlogs.CurrentReports) then
			dlogs.CurrentReports:UpdateReport(index)
		end
	end

	if dlogs.SelectedReport and dlogs.SelectedReport.index == index and ((not dlogs.SelectedReport.previous and not previous) or dlogs.SelectedReport.previous == previous) then
		dlogs.SelectedReport = updated
	end
...
net.Receive("dlogs.UpdateReports", function()
	dlogs.SelectedReport = nil
	...
	dlogs.Reports = util.JSONToTable(json)
...
local function DrawStatusMenuOption(id, menu)
	menu:AddOption(status[id], function()
		net.Start("dlogs.UpdateStatus")
		net.WriteUInt(dlogs.SelectedReport.previous and 1 or 0, 1)
		net.WriteUInt(dlogs.SelectedReport.index, 16)
		net.WriteUInt(id, 4)
		net.SendToServer()
	end):SetImage(icons[id])
end

function dlogs:DrawRDMManager(x, y)
    if (not IsValid(LocalPlayer()) or not LocalPlayer():CanUseRDMManager()) then return end
    ...
    self.CurrentReports:SetReportsTable(dlogs.Reports.Current)
    ...
    self.PreviousReports:SetReportsTable(dlogs.Reports.Previous)
    ...
    TakeActionB.Think = function(self)
        self:SetEnabled(dlogs.SelectedReport)
    end
    ...
    VictimInfos.Paint = function(panel, w, h)
        ...
        if panel.isAdmin then
            draw.SimpleText(TTTLogTranslate(GetDMGLogLang, "AdminsMessage"), "dlogs.RDM_Manager", w / 4, bar_height / 2, Color(0, 0, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        else
            draw.SimpleText(TTTLogTranslate(GetDMGLogLang, "VictimsReport"), "dlogs.RDM_Manager", w / 4, bar_height / 2, Color(0, 0, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end

        surface.SetDrawColor(220, 30, 30)
        surface.DrawRect((w / 2) + 1, 0, (w / 2), bar_height)
        draw.SimpleText(TTTLogTranslate(GetDMGLogLang, "ReportedPlayerResponse"), "dlogs.RDM_Manager", (w / 2) + 1 + (w / 4), bar_height / 2, Color(0, 0, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    ...
    KillerMessage.PaintOver = function(self, w, h)
        if self.DisableR then
            ...
            surface.SetFont("dlogs.ResponseDisabled")
    ...
    surface.SetFont("dlogs.Conclusion")
    ...
    Conclusion.PaintOver = function(panel, w, h)
        ...
        surface.SetFont("dlogs.ConclusionText")
    ...
    Conclusion.SetText = function(pnl, t)
        pnl.Text = t
        local t1, t2 = AdjustText(t, "dlogs.ConclusionText", pnl:GetWide() - cx - cm * 3)
        pnl.t1 = t1
        pnl.t2 = nil
    ...
    dlogs.UpdateReportTexts = function()
        local selected = dlogs.SelectedReport
        ...
        if selected and selected.logs then
            dlogs:SetListViewTable(VictimLogs, selected.logs, false)
        end
    end

    dlogs.DisableResponse = function(disable)
...
function PANEL:Init()
	...
	for i = 1, 3 do
		...
		local txt = i .. (i > 1 and " slays" or " slay")
	...
	self.CREnable.OnChange = function()
		self:UpdateReason()
	end
...
function PANEL:SetPlayer(reported, ply, steamid, report)
	...
	self.Button.DoClick = function(panel)
		RunConsoleCommand("mga", "aslayid", (reported and report.attacker) or (not reported and report.victim), tostring(self.NumSlays), self.CurrentReason)
...
function PANEL:Init()
	...
	self.CREnable:SetPos(self.Distance/2 + self.Dimension/2 ,self.Dimension*2/3 + 5)
	self.CREnable:SetValue(1)
	self.CREnable.OnChange = function(panel)
		self:UpdateReason()
	end
