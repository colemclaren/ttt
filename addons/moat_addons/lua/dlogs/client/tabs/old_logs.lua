...
surface.CreateFont("dlogs.OldLogsFont", {
	font = "DermaLarge",
	size = 20
})
...
local function LoadLogs(node)
	...
	net.Start("dlogs.AskOldLogRounds")
	net.WriteUInt(id, 32)
...
net.Receive("dlogs.SendOldLogRounds", function()
...
function dlogs:DrawOldLogs()
	...
	self.OldLogs.UpdateDates = function()
		...
		for y=0, years do
			...
			if not dlogs.OldLogsDays[year] then continue end
			...
			for m=start_range, end_range do
				if not dlogs.OldLogsDays[year][m] then continue end
				...
				for d=1, number_of_days do
					if not dlogs.OldLogsDays[year][m][d] then continue end
	...
	self.Olddlogs = vgui.Create("DListView", self.OldLogs)
	self.Olddlogs:SetPos(0, 280)
	self.Olddlogs:SetSize(639, 329)
	self.Olddlogs:AddColumn(TTTLogTranslate(GetDMGLogLang, "Time")):SetFixedWidth(40)
	self.Olddlogs:AddColumn(TTTLogTranslate(GetDMGLogLang, "Type")):SetFixedWidth(40)
	self.Olddlogs.EventColumn = self.Olddlogs:AddColumn(TTTLogTranslate(GetDMGLogLang, "Event"))
	self.Olddlogs.EventColumn:SetFixedWidth(529)
	self.Olddlogs.IconColumn = self.Olddlogs:AddColumn("")
	self.Olddlogs.IconColumn:SetFixedWidth(30)
	self.Olddlogs.Think = function(panel)
	...
	net.Start("dlogs.AskLogsList")
	net.SendToServer()

end

net.Receive("dlogs.SendLogsList", function()
	local received = net.ReadUInt(1) == 1
	if not received then return end
	dlogs.OldLogsDays = net.ReadTable()
	dlogs.OlderDate = net.ReadUInt(32)
	dlogs.LatestDate = net.ReadUInt(32)
	if IsValid(dlogs.OldLogs) then
		dlogs.OldLogs:UpdateDates()
	end
end)

net.Receive("dlogs.SendOldLog", function()
	...
	if exists then
		...
		if data then
			...
			if data then
				...
				if data then
					if ShotsOpen and IsValid(dlogs.OldLogsShoots) then
						dlogs.OldLogsShoots:Close()
						dlogs.OldLogsShoots:Remove()
					end
					ShotsOpen = true
					dlogs.Olddlogs:Clear()
					dlogs.OldShootTables = data.ShootTable
					dlogs:SetListViewTable(dlogs.Olddlogs, {logs = data.DamageTable, roles = data.Roles }, false, true)

					local posx, poxy = dlogs.Menu:GetPos()
					dlogs.OldLogsShoots = vgui.Create( "DFrame" )
					dlogs.OldLogsShoots:SetSize( 300, dlogs.Menu:GetTall() )
					dlogs.OldLogsShoots:SetPos( (ScrW()/2 - (dlogs.OldLogsShoots:GetWide() / 2))-500,  poxy)
					dlogs.OldLogsShoots:SetTitle( TTTLogTranslate(GetDMGLogLang, "ShotLogs") )
					dlogs.OldLogsShoots:SetDraggable( true )
					dlogs.OldLogsShoots:SetSizable(true)
					dlogs.OldLogsShoots:SetKeyboardInputEnabled(false)
					dlogs.OldLogsShoots.OnClose = function()
						ShotsOpen = false
					end

					local selectedtext = ""
					dlogs.OldShoot = vgui.Create("DListView", dlogs.OldLogsShoots)
					dlogs.OldShoot:Dock(FILL)
					dlogs.OldShoot:AddColumn(TTTLogTranslate(GetDMGLogLang, "ShotEvent"))
					dlogs.OldShoot.OnRowRightClick = function()
						...
						Menu:AddOption(TTTLogTranslate(GetDMGLogLang, "CopyLines"), function()
							local full_text = ""
							local append = false
							for _, line in pairs(dlogs.OldShoot:GetSelected()) do
								--SetClipboardText(dlogs.OldShoot:GetLine(dlogs.OldShoot:GetSelectedLine()):GetValue(1))
								if append then
									full_text = full_text .. "\n"
								end
					...
					if data.ShootTable then
						dlogs:SetDamageInfosLV(dlogs.OldShoot, data.Roles, nil, nil, nil, nil, data.ShootTable)
					end

					dlogs.PlayerList:Clear()
					for k,v in pairs(data.Roles) do
						local steamid = util.SteamIDFrom64(v.steamid64)
						local item = dlogs.PlayerList:AddLine(v.nick, steamid, dlogs:StrRole(v.role))
...
function dlogs:FindFromOldLogs(t, att, victim)
	local results = {}
	local found = false
