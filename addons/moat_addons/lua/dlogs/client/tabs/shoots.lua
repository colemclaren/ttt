function dlogs:DrawShootsTab()
	...
	self.ShootsList.OnRowRightClick = function()
		...
		Menu:AddOption(TTTLogTranslate(GetDMGLogLang, "CopyLines"), function()
			local full_text = ""
			local append = false
			for _, line in pairs(dlogs.ShootsList:GetSelected()) do
				if append then
					full_text = full_text .. "\n"
				end
...
net.Receive("dlogs.SendShootLogs", function()
	local roles = net.ReadTable()
	local data = net.ReadTable()
	if IsValid(dlogs.ShootsList) then
		dlogs.ShootsList:Clear()
		dlogs:SetDamageInfosLV(dlogs.ShootsList, roles, nil, nil, nil, nil, data)
	end
end)
