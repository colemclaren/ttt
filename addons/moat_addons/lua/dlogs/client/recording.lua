...
surface.CreateFont("dlogs.Shots_Title", {
	font = "DermaLarge",
	size = 18
})
...
local function CreateShotsPanel()
	...
	Title:SetPos(0, 10)
	Title:SetFont("dlogs.Shots_Title")
	...
	Frame.SetInfo = function(Frame)
		...
		for i=1, size do
			...
			for id, tbl in ipairs(moment) do
				...
				local nick = dlogs:InfoFromID(roles, id).nick
				local wep = dlogs:GetWeaponName(tbl.wep) or TTTLogTranslate(GetDMGLogLang, "UnknownWeapon")
...
hook.Add("Initialize", "DeathSceneFrame", function()
	...
	dlogs.DeathSceneInitialized = true
end)

if dlogs.DeathSceneInitialized then
	Frame = CreateShotsPanel()
end
...
net.Receive("dlogs.SendDeathScene", function()
...
hook.Add("RenderScreenspaceEffects", "DeathScene_dlogs", function()
	...
	if current_scene then
		...
		for id, model in pairs(models) do
			...
			if model.corpse then
				...
				if wep then
					local wepEntityName = dlogs:GetWeaponName(wep) or wep
				...
				nick = nick .. " [" .. dlogs:StrRole(model.role) .. "]"
...
hook.Add("Think", "Think_Record", function()
	...
	if current_scene and (not last_curtime or (last_curtime and (CurTime() - last_curtime) >= 0.01)) then
		...
		dlogs.DS_Progress.TranslateValues = function(self, x, y)
			i = #current_scene * x

			return x, y
		end

		dlogs.DS_Replay.DoClick = function()
			i = 1
		end
		...
		dlogs.DS_Progress:SetSlideX(progress / #current_scene)
		dlogs.DS_Progress:SetSlideY(0.5)
		...
		if scene then
			...
			if not paused and dlogs.DS_Play.Icon:GetImage() == "icon16/control_pause_blue.png" then
				dlogs.DS_Play:DoClick()
			end
		end
...
function dlogs:IsRecording()
	return current_scene and true or false
end

function dlogs:StopRecording()
	self.DSPanel:Remove()
	net.Start("dlogs.UpdateLogEnt")
	net.WriteUInt(0, 1)
	net.SendToServer()
