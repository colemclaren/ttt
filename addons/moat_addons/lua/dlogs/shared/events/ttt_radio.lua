...
if SERVER then
	dlogs:EventHook("TTTPlayerRadioCommand")
else
	dlogs:AddFilter("filter_show_radiocommands", DAMAGELOG_FILTER_BOOL, false)
	dlogs:AddFilter("filter_show_radiokos", DAMAGELOG_FILTER_BOOL, false)
	dlogs:AddColor("color_defaultradio", Color(182,182,182))
	dlogs:AddColor("color_kosradio", Color(255,0,0))
end
...
function event:ToString(v, roles)
	...
	local targetrole = ""
	if targetply then
		targetrole = " ["..dlogs:StrRole(v[4]).."]"
	end
	local ply = dlogs:InfoFromID(roles, v[1])
	return string.format(TTTLogTranslate(GetDMGLogLang, "RadioUsed"), ply.nick, dlogs:StrRole(ply.role), text, targetrole)
end

function event:IsAllowed(tbl, roles)
	local ply = dlogs:InfoFromID(roles, tbl[1])
	if tbl[2] != "quick_traitor" then
		return dlogs.filter_settings["filter_show_radiocommands"]
	else
		return dlogs.filter_settings["filter_show_radiokos"]
	end
end

function event:Highlight(line, tbl, text)
	if table.HasValue(dlogs.Highlighted, tbl[1]) or table.HasValue(dlogs.Highlighted, tbl[5]) then
		return true
	end
	return false
end
...
function event:GetColor(tbl, roles)
	local ply = dlogs:InfoFromID(roles, tbl[1])
	if tbl[2] != "quick_traitor" then
		return dlogs:GetColor("color_defaultradio")
	else
		return dlogs:GetColor("color_kosradio")
	end
end
