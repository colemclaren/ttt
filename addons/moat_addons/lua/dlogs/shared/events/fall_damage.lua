if SERVER then
	dlogs:EventHook("EntityTakeDamage")
else
	dlogs:AddFilter("filter_show_falldamage", DAMAGELOG_FILTER_BOOL, false)
	dlogs:AddColor("color_fall_damages", Color(0, 0, 0))
end
...
function event:ToString(tbl, roles)
	...
	local t = string.format(TTTLogTranslate(GetDMGLogLang, "FallDamage"), ply.nick, dlogs:StrRole(ply.role), tbl[2])
	if tbl[3] then
		local att = dlogs:InfoFromID(roles, tbl[4])
		t = t..string.format(TTTLogTranslate(GetDMGLogLang, "AfterPush"), att.nick, dlogs:StrRole(att.role))
	end
	return t
end

function event:IsAllowed(tbl)
	return dlogs.filter_settings["filter_show_falldamage"]
end
...
function event:GetColor(tbl)
	if tbl[5] and dlogs:IsTeamkill(tbl[2], tbl[7]) then
		return dlogs:GetColor("color_team_damages")
	else
		return dlogs:GetColor("color_fall_damages")
	end
end
