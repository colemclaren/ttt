-- https://github.com/SuperiorServers/dash/blob/master/lua/dash/libraries/server/nomr.mysql.lua

--include 'no_multirun.lua'
require 'tmysql4'

nomr.mysql = setmetatable({
...
function nomr.mysql.Connect(hostname, username, password, database, port, optional_socketpath, optional_clientflags)
	...
	if nomr.mysql.GetTable[tostring(db_obj)] then
		return nomr.mysql.GetTable[tostring(db_obj)]
	end
	...
	elseif (db_obj.Handle == false) then
		...
		nomr.mysql.GetTable[tostring(db_obj)] = db_obj
...
timer.Create('nomr.mysql.QueryQueue', 0.5, 0, function()
