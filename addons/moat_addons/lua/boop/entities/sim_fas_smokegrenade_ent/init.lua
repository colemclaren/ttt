...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("sim_fas_smokegrenade_ent")
		ent:SetPos(SpawnPos)
	ent:Spawn()
	ent:Activate()
...
function ENT:Use(activator, caller)
	...
	if (activator:IsPlayer()) then
		if activator:GetWeapon("weapon_fas_grenadesmoke") == NULL then
			activator:Give("weapon_fas_grenadesmoke")
		else
			activator:GiveAmmo(1, "smokegrenade")
		end
	end
end