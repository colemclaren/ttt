...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("sim_fas_flare_ent")
		ent:SetPos(SpawnPos)
	ent:Spawn()
	ent:Activate()
...
function ENT:Use(activator, caller)
	...
	if (activator:IsPlayer()) then
		if activator:GetWeapon("weapon_fas_grenadeflare") == NULL then
			activator:Give("weapon_fas_grenadeflare")
		else
			activator:GiveAmmo(1, "flaregrenade")
		end
	end
end