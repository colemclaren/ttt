...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("sim_fas_fraggrenade_ent")
		ent:SetPos(SpawnPos)
	ent:Spawn()
	ent:Activate()
...
function ENT:Use(activator, caller)
	...
	if (activator:IsPlayer()) then
		if activator:GetWeapon("weapon_fas_grenadefrag") == NULL then
			activator:Give("weapon_fas_grenadefrag")
		else
			activator:GiveAmmo(1, "fraggrenade")
		end
	end
end