...
local MOAT_CUR_DRAGON_BOSS = false
local MOAT_BOSS_DMG = {}
...
local function moat_DrawBossHealth()
	...
	local col = {
		[7] = {Color(255, 255, 255), Color(255, 255, 255)}
	}

	for i = 1, 2 do
		draw.SimpleText(text, font, x + (w / 2) + i, y + (h / 2) + i, rarity_shadow[7][i], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.SimpleText(text, font, x + (w / 2) - i, y + (h / 2) - i, rarity_shadow[7][i], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.SimpleText(text, font, x + (w / 2) + i, y + (h / 2) - i, rarity_shadow[7][i], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.SimpleText(text, font, x + (w / 2) - i, y + (h / 2) + i, rarity_shadow[7][i], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end

	draw.SimpleText(text, "TimeLeft", x + (w / 2), y + (h / 2), rarity_accents[7], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
local function moat_InitDrawBossHealth()
	...
	cdn.PlayURL("https://static.moat.gg/ttt/dragon.mp3", .8, function() end, "stream")
end
...
local function moat_DrawBossEnd(MOAT_BOSS_LOSS)
	...
	local size_change = math.abs(math.sin((RealTime() - (1 * 0.1)) * 1.5))
	draw.RoundedBox(0, (ScrW() / 2) - (textw / 2), 100 + texth + 15, textw, 10, Color(255, 0, 0, 255 * size_change))
	draw.RoundedBox(0, (ScrW() / 2) - (textw / 2), 100 - 25, textw, 10, Color(255, 0, 0, 255 * size_change))
...
net.Receive("Dragon.Prep", function(len)
	moat_PrepareBoss()
	MOAT_BOSS_DMG = {}
end)

net.Receive("Dragon.Begin", function(len)
	local ent = net.ReadEntity()
	local ply = net.ReadEntity()
	local bosspos = net.ReadVector()
	MOAT_CUR_BOSS = ent
	MOAT_CUR_BOSS_PLY = ply
	MOAT_CUR_DRAGON_BOSS = true
	MOAT_BOSS_ROUND_OVER = false
	moat_InitDrawBossHealth()
	local the_pos = bosspos + Vector(0, 0, 400)

	hook.Add("CalcView", "moat_SpectateDragon", function(ply, pos, angles, fov)
		if (not MOAT_CUR_DRAGON_BOSS) then
			hook.Remove("CalcView", "moat_SpectateDragon")
...
net.Receive("Dragon.End", function(len)
