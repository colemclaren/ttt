...
local function moat_DrawBossHealth()
    ...
    m_DrawShadowedText(1, "Team up and kill the DRAGON!", "moat_ItemDesc", (ScrW()/2), 30, Color(200, 200, 200, 255), TEXT_ALIGN_CENTER)
...
local function moat_InitDrawBossHealth()
	...
	--cdn.PlayURL("https://static.moat.gg/f/0s40VLHeGvnARu7ltnszS2EujXPX.mp3", 0.5)
end
...
local function moat_PrepareBoss()
	...
	--cdn.PlayURL("https://static.moat.gg/f/NbpXvhyZPp2LMNf1qbaj2pgl7Qko.mp3")
...
net.Receive("MOAT_PREP_DRAGON", function(len)
	moat_PrepareBoss()
	MOAT_BOSS_DMG = {}
end)

net.Receive("MOAT_BEGIN_DRAGON", function(len)
	...
	hook.Add("CalcView", "moat_FocusBossView", function(ply, pos, angles, fov)
		...
		view.origin = the_pos-(angles:Forward()*500) + Vector(0, 0, 200)
		view.angles = angles
		view.fov = fov
...
net.Receive("MOAT_END_DRAGON", function(len)
