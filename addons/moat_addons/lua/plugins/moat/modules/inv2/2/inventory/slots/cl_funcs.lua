...
function mi:SlotExists(slot, fn)
	...
	self.SQL:Query(query, function(d)
		...
		if (d and d[1] and d[1].c == "0") then
			self.SQL:Query("DELETE FROM "..ck.." WHERE c = 0;", Internal)
		else
			Internal()
		end
	end)
...
function mi:SaveSlotItem(slot, id, fn)
	return self:SlotExists(slot, function(exists)
		...
		local query = self.SQL:CreateQuery([[
			INSERT INTO ]]..ck..[[ (c, slotid, steamid) VALUES (?, ?, ?!);
		]], id, slot, steamid())
...
function mi:MassSwapInventory(new_slots, fn)
	return self:GetOurSlots(function(max, cache)
		...
		local query = "DELETE FROM "..ck..";\n"
		local template = "INSERT INTO "..ck.." (c, slotid, steamid) VALUES "
...
function mi:SwapSlotItem(slot, slot2, fn)
	return self:GetOurSlots(function(max, cache)
		...
		local query = self.SQL:CreateQuery(item2 ~= 0 and [[
			UPDATE ]]..ck..[[ SET c = ?item2 WHERE slotid = ?slot1 AND steamid = ?!steamid;
			UPDATE ]]..ck..[[ SET c = ?item1 WHERE slotid = ?slot2 AND steamid = ?!steamid;
		]] or [[
			DELETE FROM ]]..ck..[[ WHERE slotid = ?slot1 and steamid = ?steamid!;
			REPLACE INTO ]]..ck..[[ (c, slotid, steamid) VALUES (?item1, ?slot2, ?!steamid);
		]], {
			slot1 = slot,
			slot2 = slot2,
			item1 = item1,
...
function mi:GetOurSlots(fn)
	...
	local query = self.SQL:CreateQuery("SELECT slotid, c FROM "..ck.." WHERE steamid = ?!;", steamid())
