...
function MOAT_LOADOUT.UpdateWep()
	local wep_index, wep_d, wep_f, wep_m, wep_r, wep_a, wep_v, wep_p, wep_stats
    ...
    timer.Create("moat_StatRefresh" .. wep_index, 0.1, 0, function()
		...
		if (prim) then
            if (prim.ClipSize) then
                wep.Weapon.Primary.ClipSize = wep_m

                if (prim.DefaultClip) then wep.Weapon.Primary.DefaultClip = wep_m end
                if (prim.ClipMax) then wep.Weapon.Primary.ClipMax = (wep_m * 3) end
            end

			if (prim.Damage) then wep.Weapon.Primary.Damage = wep_d end
            if (prim.Delay) then wep.Weapon.Primary.Delay = wep_f end
            if (prim.Recoil) then wep.Weapon.Primary.Recoil = wep_r end
            if (prim.Cone) then wep.Weapon.Primary.Cone = wep_a end
		end
	
        if (wep.Weapon.PushForce) then wep.Weapon.PushForce = wep_v end
        if (wep.Weapon.Secondary.Delay) then wep.Weapon.Secondary.Delay = wep_p end
