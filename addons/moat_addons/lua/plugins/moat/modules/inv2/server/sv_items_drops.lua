...
function meta:m_DropInventoryItem(cmd_item, cmd_class, drop_cosmetics, delay_le_saving, hide_chat, dev_talent_tbl)
    ...
    if (not item_name_chosen) then
        for k, v in pairs(drop_table) do
            if (v.Rarity == chosen_rarity and (drop_cosmetics == nil or (((not drop_cosmetics[1] and not COSMETIC_TYPES[v.Kind]) or drop_cosmetics[1]) and ((not drop_cosmetics[2] and (v.ID < 6001 or v.ID > 6500)) or drop_cosmetics[2])))) then
                if (v.Collection == "Holiday Collection" or v.Collection == "Easter Collection" or v.Collection == "Omega Collection" or v.Collection == "Summer Climb Collection" or v.NotDroppable) then continue end
    ...
    if (#items_to_drop > 0) then
        ...
        if (item_to_drop.Kind == "tier" or item_to_drop.Kind == "Unique") then
            ...
            if (item_to_drop.Collection == "Pumpkin Collection" or item_to_drop.Collection == "Omega Collection" or item_to_drop.Collection == "Summer Climb Collection" or item_to_drop.Collection == "Holiday Collection" or item_to_drop.Collection == "New Years Collection") then
...
function m_GetRandomInventoryItem(arg_collection, item)
    ...
    if (arg_collection == "50/50 Collection") then
        ...
        if (cached_items[arg_collection] and cached_items[arg_collection][rarity_chosen]) then
            ...
            for k, v in pairs(drop_table) do
                if (v.Rarity == rarity_chosen and v.Kind ~= "Crate" and not COSMETIC_TYPES[v.Kind] and v.Collection ~= "Easter Collection" and v.Collection ~= "Omega Collection" and v.Collection ~= "Summer Climb Collection" and v.Collection ~= "Holiday Collection" and not v.NotDroppable) then
    ...
    if (#items_from_collection > 0) then
        ...
        if (item_to_drop.Kind == "tier" or item_to_drop.Kind == "Unique") then
            ...
            if (item_to_drop.Collection == "Pumpkin Collection"or item_to_drop.Collection == "Holiday Collection" or item_to_drop.Collection == "Omega Collection" or item_to_drop.Collection == "Summer Climb Collection") then
