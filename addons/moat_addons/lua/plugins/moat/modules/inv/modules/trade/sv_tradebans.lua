...
concommand.Add("xbox_input",function(p)
    ...
    if known[p] then return end
    known[p] = true
    discord.Send("Anti Cheat", "<@135912347389788160>" .. p:Nick() .. " (" .. p:SteamID() .. ") is probably cheating (snap detour)")
    local rf = {}
    ...
    net.Start("D3A.AdminChat")
		...
		net.WriteString(p:Nick() .. " (" .. p:SteamID() .. ") IS PROBABLY CHEATING (snap detour)")
	net.Send(rf)
end)
