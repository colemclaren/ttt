...
COMMAND.Run = function(pl, args, supplement)
	...
	if (rounds >= 1) then
		D3A.Chat.Broadcast2(pl, moat_teal, plname, moat_white, " has added ", moat_green, tostring(rounds), moat_white, " autoslays to ", moat_green, targ, moat_white, " with the reason: ", moat_green, reason, moat_white, ".")
		D3A.Commands.Discord("aslay", D3A.Commands.NameID(pl), rounds, "Offline Player (" .. targ .. ")", reason)
	else
		D3A.Chat.Broadcast2(pl, moat_teal, plname, moat_white, " has removed the autoslays of ", moat_green, targ, moat_white, ".")
