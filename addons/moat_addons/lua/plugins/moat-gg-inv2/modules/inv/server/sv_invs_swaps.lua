...
local function GMEnumsForLoadoutSlots()
...
hook("Initialize", GMEnumsForLoadoutSlots)
GMEnumsForLoadoutSlots()
...
net.Receive("Moat.Swap", function(len, pl)
    ...
    local query = mi:CreateQuery("UPDATE moat_inv_items SET slotid = NULL WHERE slotid = ? AND ownerid = ?;", slotid, pl)
    if (new_wep ~= 0) then
        query = query .. mi:CreateQuery("UPDATE moat_inv_items SET slotid = ? WHERE ownerid = ? AND id = ?;", slotid, pl, new_wep)
    end
