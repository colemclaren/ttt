...
function meta:m_AddInventoryItem(tbl, _, no_chat)
    self:AddItem(tbl, function(id)
        ...
        mi.WriteWeaponsToPlayer(targets, {tbl}, function(ply)
			if (self == ply) then
                net.Start("Moat.AddInvItem")
                net.WriteUInt(tbl.c, 32)
                net.WriteBool(no_chat or false)
                net.Send(self)
                if (not no_chat) then
                    net.Start("Moat.NewItem")
                    	net.WriteUInt(tbl.c, 32)
                    net.Send(self)
                end
            end
...
net.Receive("Moat.InvCat", function(len, ply)
    local inv_cat = net.ReadByte()
    net.Start("Moat.InvCat")
		net.WriteByte(inv_cat)
