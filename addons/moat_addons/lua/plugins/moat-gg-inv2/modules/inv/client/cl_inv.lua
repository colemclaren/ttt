...
local m_Credits = 0
MOAT_INVENTORY_CREDITS = m_Credits
net.Receive("Moat.SendCredits", function(len)
    m_Credits = math.Round(net.ReadDouble(), 2)
    MOAT_INVENTORY_CREDITS = m_Credits
end)
...
function m_OpenInventory(ply2, utrade)
    ...
    for k, v in ipairs(M_INV_CATS) do
        ...
        MOAT_CAT_BTN.DoClick = function(s)
            ...
            net.Start("Moat.InvCat")
				net.WriteByte(s.CAT_NUM)
            net.SendToServer()
        end
    ...
    local function m_CreateInvSlotInternal(num)
		...
		local vistop, visbottom = (ScrH() / 2) - (MOAT_INV_BG_H / 2), (ScrH() / 2) + (MOAT_INV_BG_H / 2)
        m_DPanel.Paint = function(s, w, h)
			local px, py = s:LocalToScreen(0, 0)
			if (py < vistop or py > visbottom) then
				-- print "yeet"
				return
			end
    ...
    function m_CreateLoadoutSlot(num)
        ...
        m_DPanel.Paint = function(s, w, h)
            ...
            if (m_Loadout[num].c and m_Loadout[num].item and m_Loadout[num].item.Rarity) then
                surface_SetDrawColor(150 + (hover_coloral / 2), 150 + (hover_coloral / 2), 150 + (hover_coloral / 2), 100)
                surface_DrawRect(draw_x, draw_y, draw_w, draw_h)
    ...
    MOAT_INV_S.Paint = function(s, w, h)
        ...
        if (ITEM_HOVERED and ITEM_HOVERED.c) then
            ...
            if (((ITEM_HOVERED.s or (ITEM_HOVERED.u and ITEM_HOVERED.u == 0)) or (ITEM_HOVERED.u and ITEM_HOVERED.u == 0)) and (ITEM_HOVERED.item.Kind == "tier" or ITEM_HOVERED.item.Kind == "Unique" or ITEM_HOVERED.item.Kind == "Melee")) then
                draw_stats_multi = 25
...
net.Receive("Moat.InvCat", function(len)
    local inv_cat = net.ReadByte()
...
net.Receive("Moat.UpdateEXP", function(len)
...
net.Receive("Moat.SendSlots", function(len)
    NUMBER_OF_SLOTS = net.ReadShort(16)
