...
if GetConVar("DavyCrockettAllowed") == nil then
    ...
    if (GetConVar("DebugM9K"):GetBool()) then
        print("m9k_davy_crockett blacklist convar created!")
    end
end
...
if not game.SinglePlayer() then
    ...
    if SERVER then
        function ClientSideGasDisabler()
            timer.Create("ClientGasBroadcastTimer", 15, 0, function()
                BroadcastLua("RunConsoleCommand(\"M9KGasEffect\", \"0\")")
            end)
        end
...
--if one of them doesn't exists, then they all probably don't exist
if GetConVarString("nuke_yield") == nil then
