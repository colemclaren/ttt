...
dlogs.ReportsQueue = dlogs.ReportsQueue or {}
local ReportFrame

local function BuildReportFrame(report)

	if IsValid(ReportFrame) and report then
		for k,v in pairs(dlogs.ReportsQueue) do
		...
		ReportFrame.AddReport = function(ReportFrame, report)
			...
			local Info = vgui.Create("dlogs_InfoLabel")
			local txt = ""
			...
			Button.DoClick = function()
				...
				if size < 10 then
					...
					ReportFrame:Close()
					ReportFrame:Remove()
					dlogs:Notify(DAMAGELOG_NOTIFY_INFO, TTTLogTranslate(GetDMGLogLang, "ResponseSubmitted"), 4, "")
				end
			end
		...
		for _, report in ipairs(dlogs.ReportsQueue) do
...
vgui.Register("dlogs_Player", PANEL, "DPanel")

function dlogs:ReportWindow(found, deathLogs, previousReports, currentReports, dnas)
	...
	local InfoLabel = vgui.Create("dlogs_InfoLabel", ReportPanel)
	...
	UserList.AddPlayer = function(pnl, pl, is_killer, killer_valid)
		...
		local ply = vgui.Create("dlogs_Player")
		ply:SetSize(0, 30)
		ply:SetPlayer(pl)
	...
	VictimInfos.Paint = function(panel, w, h)
		...
		draw.SimpleText(TTTLogTranslate(GetDMGLogLang, "YourReport"), "dlogs.RDM_Manager", w / 4, bar_height / 2, Color(0, 0, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		surface.SetDrawColor(220, 30, 30)
		surface.DrawRect((w / 2) + 1, 0, (w / 2), bar_height)
		draw.SimpleText(TTTLogTranslate(GetDMGLogLang, "PlayerResponse"), "dlogs.RDM_Manager", (w / 2) + 1 + (w / 4), bar_height / 2, Color(0, 0, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	...
	status:SetPos(10, 10)
	status:SetFont("dlogs.RDM_Manager")
	...
	curStatus:SetFont("dlogs.RDM_Manager")
	...
	conclusion:SetFont("dlogs.RDM_Manager")
	...
	conclusionText:SetFont("dlogs.RDM_Manager")
	...
	if #previousReports > 0 then
		...
		for _,report in ipairs(previousReports) do
			local button = vgui.Create("dlogs_Player")
			...
			button.OnSelected = function(button)
				...
				net.Start("dlogs.AskOwnReportInfo")
				net.WriteUInt(1, 1)
...
net.Receive("dlogs.SendOwnReportInfo", function()
	local tbl = net.ReadTable()
	if not IsValid(dlogs.ReportsInfo) then return end
	if tbl.previous and not dlogs.ReportsInfo.ViewingPrevious then return end
	if tbl.index != dlogs.ReportsInfo.CurrentIndex then return end
	dlogs.ReportsInfo:ViewLogs(tbl)
end)

net.Receive("dlogs.AllowReport", function()
	...
	dlogs:ReportWindow(found,deathLogs, previousReports, currentReports, dnas)
end)

net.Receive("dlogs.SendReport", function()
	local report = net.ReadTable()

	if IsValid(ReportFrame) then
		BuildReportFrame(report)
		dlogs.ReportsQueue[#dlogs.ReportsQueue + 1] = report
	else
		dlogs.ReportsQueue[#dlogs.ReportsQueue + 1] = report
...
net.Receive("dlogs.Death", function()
	if not IsValid(ReportFrame) then
		BuildReportFrame()
	end
end)

net.Receive("dlogs.SendForgive", function()
	...
	if canceled then
		local InfoLabel = vgui.Create("dlogs_InfoLabel", answer)
...
net.Receive("dlogs.Answering_global", function(_len)
...
surface.CreateFont("dlogs.PendingNumber", {
	font = "DermaLarge",
	size = 25
})

surface.CreateFont("dlogs.PendingText", {
	font = "DermaLarge",
	size = 16
})
...
hook.Add("HUDPaint", "dlogsPendingReports", function()
	...
	local alpha = #dlogs.Notifications > 0 and 30 or 255

	if not IsValid(syncEnt) then
		syncEnt = dlogs:GetSyncEnt()
	...
	surface.SetFont("dlogs.PendingText")
	...
	surface.SetFont("dlogs.PendingNumber")
