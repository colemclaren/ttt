surface.CreateFont("dlogs.ChatCategory", {
    font = "DermaDefault",
    size = 17,
    weight = 700
})

surface.CreateFont("dlogs.ChatFont", {
    font = "DermaDefault",
    size = 17
})

surface.CreateFont("dlogs.ChatPlayer", {
    font = "DermaDefault",
    size = 16,
    weight = 600
})
...
function PANEL:Paint(w, h)
    ...
    surface.SetFont("dlogs.ChatPlayer")
...
vgui.Register("dlogs.ChatPlayer", PANEL, "DPanel")
local PANEL = {}
...
function PANEL:AddPlayer(ply, playertype)
    if not table.HasValue(self.Players, ply) then
        local panel = vgui.Create("dlogs.ChatPlayer", self)
        panel:SetHeight(30)
...
function PANEL:Paint()
    ...
    surface.SetFont("dlogs.ChatCategory")
    surface.SetTextColor(Color(150, 150, 150))
    surface.SetTextPos(5, 5)
    surface.DrawText(self.Name)
end

vgui.Register("dlogs.ChatCategory", PANEL, "DPanel")
local PANEL = {}

function PANEL:Init()
    self:EnableVerticalScrollbar(true)
    self.Normal = vgui.Create("dlogs.ChatCategory", self)
    self.Normal:SetCategoryName(TTTLogTranslate(GetDMGLogLang, "ChatPlayers"))
    self.Admins = vgui.Create("dlogs.ChatCategory", self)
...
vgui.Register("dlogs.ChatList", PANEL, "DPanelList")
dlogs.CurrentChats = dlogs.CurrentChats or {}

function dlogs:StartChat(report, admins, victim, attacker, players, history, adminReport)
    ...
    local List = vgui.Create("dlogs.ChatList", Chat)
    ...
    Actions.DoClick = function(self)
        ...
        for k, v in pairs(List.Normal.Players) do
            local playerPanel = forceStayList:AddOption(IsValid(v) and v:Nick() or TTTLogTranslate(GetDMGLogLang, "ChatDisconnected"), function()
                if not IsValid(v) then
                    dlogs:Notify(DAMAGELOG_NOTIFY_ALERT, TTTLogTranslate(GetDMGLogLang, "InvalidPlayerChat"), 2, "weapon_cant_buy.wav")
        ...
        menu:AddOption("Leave chat", function()
            if #List.Admins.Players <= 1 then
                dlogs:Notify(DAMAGELOG_NOTIFY_ALERT, TTTLogTranslate(GetDMGLogLang, "CannotLeaveChat"), 4, "weapon_cant_buy.wav")
            else
                Chat:Close()
                net.Start("dlogs.LeaveChat")
                net.WriteUInt(Chat.RID, 32)
                net.SendToServer()

                for k, v in pairs(dlogs.CurrentChats) do
                    if v == Chat then
                        table.remove(dlogs.CurrentChats, k)
                        break
                    end
                end
    ...
    RichText.AddText = function(self, nick, color, text)
        self.m_FontName = "dlogs.ChatFont"
        self:SetFontInternal("dlogs.ChatFont")
    ...
    Chat.AddMessage = function(self, msg)
        RichText.m_FontName = "dlogs.ChatFont"
        RichText:SetFontInternal("dlogs.ChatFont")
    ...
    local function SendMessage(msg)
        ...
        net.Start("dlogs.SendChatMessage")
...
net.Receive("dlogs.BroadcastMessage", function()
...
net.Receive("dlogs.OpenChat", function()
    ...
    dlogs:StartChat(report, {admin}, victim, attacker, players, history, adminReport)
end)

net.Receive("dlogs.JoinChatCL", function()
    ...
    if is_joining then
        ...
        dlogs:StartChat(id, tbl.admins, tbl.victim, tbl.attacker, tbl.players, history)
    else
        local id = net.ReadUInt(32)
        local ply = net.ReadEntity()
        local category = net.ReadUInt(32)
        local chat = dlogs.CurrentChats[id]
...
hook.Add("TTTBeginRound", "dlogs_Chat", function()
    drawing = false
end)
...
hook.Add("Think", "dlogs_Chat", function()
...
net.Receive("dlogs.StopChat", function()
    local id = net.ReadUInt(32)
...
net.Receive("dlogs.LeaveChatCL", function()
    local id = net.ReadUInt(32)
...
net.Receive("dlogs.ForcePlayerStay", function()
    local id = net.ReadUInt(32)
...
net.Receive("dlogs.ReleaseCL", function()
    local id = net.ReadUInt(32)
...
net.Receive("dlogs.ForceStayNotification", function()
    local id = net.ReadUInt(32)
...
net.Receive("dlogs.ViewChatCL", function()
    local id = net.ReadUInt(32)
    local length = net.ReadUInt(32)
    local compressed = net.ReadData(length)
    local json = util.Decompress(compressed)
...
hook.Add("HUDPaint", "dlogs_Chat", function()
    if IsValid(LocalPlayer()) then
        local wr, hr = 150, 40
        local w, h = ScrW() / 2, ScrH() - 50

        if not drawing and #dlogs.CurrentChats > 0 then
            TIPS.Hide()
            drawing = true

            if dlogs.ChatButton then
                dlogs.ChatButton:Remove()
            end

            dlogs.ChatButton = vgui.Create("DButton")
            dlogs.ChatButton:SetSize(16, 16)
            dlogs.ChatButton:SetPos(w + wr / 2 - dlogs.ChatButton:GetWide() - 10, h - dlogs.ChatButton:GetTall() / 2)
            dlogs.ChatButton:SetText("")

            dlogs.ChatButton.PaintOver = function(self, w, h)
            ...
            dlogs.ChatButton.DoClick = function(self)
                show_chats = not show_chats
            end
        elseif drawing and #dlogs.CurrentChats == 0 then
        ...
        if #dlogs.CurrentChats > 0 then
            ...
            surface.SetTextColor(color_black)
            surface.SetFont("dlogs.ChatCategory")
            local text = string.format(TTTLogTranslate(GetDMGLogLang, "ActiveChats"), #dlogs.CurrentChats)
            local wt, ht = surface.GetTextSize(text)
            ...
            if missing_messages > 0 and not show_chats then
                ...
                dlogs.DrawCircle(w + wr / 2, h - hr / 2, 13, 50)
                surface.SetFont("dlogs.ChatCategory")
            ...
            if show_chats then
                ...
                for k, v in ipairs(dlogs.CurrentChats) do
                    if k == 1 then
                        i = i - 17
                    else
                        i = i - 27
                    ...
                    surface.SetDrawColor(Color(92, 127, 183))
                    dlogs.DrawCircle(_x + max_w - 15, _y + 15, 10, 20)
                    surface.SetTextColor(color_white)
                    surface.SetFont("dlogs.ChatPlayer")
