...
local function CreateCommand()
	...
	function ulx.autoslayid(calling_ply, target, rounds, reason)
		if ULib.isValidSteamID(target) then
			...
			dlogs:SetSlays(calling_ply, target, rounds, reason, false)
		else
	...
	autoslay:addParam({
		...
		default = dlogs.Autoslay_DefaultReason,
		ULib.cmds.optional,
...
if CLIENT then
	...
	net.Receive("dlogs.SlayMessage", dlogs.SlayMessage)

	net.Receive("dlogs.AutoSlay", function()
	...
	net.Receive("dlogs.AutoSlaysLeft", function()
		local ply = net.ReadEntity()
	...
	net.Receive("dlogs.PlayerLeft", function()
	...
	local ents = {}
	net.Receive("dlogs.SendJails", function()
