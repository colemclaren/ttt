...
if not dlogs.Reports.Previous then
    if file.Exists("damagelog/prevreports.txt", "DATA") then
        dlogs.Reports.Previous = util.JSONToTable(file.Read("damagelog/prevreports.txt", "DATA"))
...
local function UpdatePreviousReports()
    local tbl = table.Copy(dlogs.Reports.Current)
...
function Player:UpdateReports()
    ...
    local tbl = util.TableToJSON(dlogs.Reports)
...
hook.Add("PlayerSay", "dlogs_RDMManager", function(ply, text, teamOnly)
...
hook.Add("TTTBeginRound", "dlogs_RDMManger", function()
    for k, v in ipairs(player.GetHumans()) do
...
hook.Add("TTTFoundDNA", "dlogs", OnDNAFound)
...
net.Receive("dlogs.ReportPlayer", function(_len, ply)
    ...
    message = string.gsub(string.gsub(message, "[^%g\128-\191\194-\197\208-\210 ]+", ""), "%s+", " ")
    ...
    if not ply:CanUseRDMManager() then
        ...
        if not dlogs.Config.NoStaffReports then
        ...
        if not ply.CanReport then
            if not dlogs.Config.MoreReportsPerRound then
                ply:dlogs_Notify(DAMAGELOG_NOTIFY_ALERT, TTTLogTranslate(ply.DMGLogLang, "NeedToPlay"), 4, "weapon_cant_buy.wav")

                return
            end
        else
            if not dlogs.Config.ReportsBeforePlaying then
    ...
    local index = table.insert(dlogs.Reports.Current, newReport)
    ...
    for k, v in ipairs(player.GetHumans()) do
        if v:CanUseRDMManager() then
            if v:IsActive() then
                ...
                if reportType ~= DAMAGELOG_REPORT_STANDARD then
                    v:dlogs_Notify(DAMAGELOG_NOTIFY_ALERT, string.format(TTTLogTranslate(v.DMGLogLang, "HasAdminReported"), ply:Nick(), attacker:Nick(), index), 5, "vote_failure.wav")
                else
                    v:dlogs_Notify(DAMAGELOG_NOTIFY_ALERT, string.format(TTTLogTranslate(v.DMGLogLang, "HasReported"), ply:Nick(), attacker:Nick(), index), 5, "vote_failure.wav")
                end
            end
    ...
    if not attacker:CanUseRDMManager() then
        if reportType ~= DAMAGELOG_REPORT_STANDARD then
            attacker:dlogs_Notify(DAMAGELOG_NOTIFY_ALERT, string.format(TTTLogTranslate(attacker.DMGLogLang, "HasAdminReportedYou"), ply:Nick()), 5, "vote_failure.wav")
        else
            attacker:dlogs_Notify(DAMAGELOG_NOTIFY_ALERT, string.format(TTTLogTranslate(attacker.DMGLogLang, "HasReportedYou"), ply:Nick()), 5, "vote_failure.wav")
        end
    end

    if reportType ~= DAMAGELOG_REPORT_STANDARD then
        ply:dlogs_Notify(DAMAGELOG_NOTIFY_ALERT, string.format(TTTLogTranslate(ply.DMGLogLang, "YouHaveAdminReported"), attacker:Nick()), 5, "")
    else
        ply:dlogs_Notify(DAMAGELOG_NOTIFY_ALERT, string.format(TTTLogTranslate(ply.DMGLogLang, "YouHaveReported"), attacker:Nick()), 5, "")
    end
    ...
    if reportType == DAMAGELOG_REPORT_CHAT then
        ...
        for k, v in ipairs(player.GetHumans()) do
            if v:CanUseRDMManager() then
                v:dlogs_Notify(DAMAGELOG_NOTIFY_INFO, string.format(TTTLogTranslate(v.DMGLogLang, "OpenChatNotification"), ply:Nick(), index), 5, "")
...
net.Receive("dlogs.UpdateStatus", function(_len, ply)
    ...
    if status == RDM_MANAGER_WAITING then
        msg = string.format(TTTLogTranslate(ply.DMGLogLang, "HasSetReport"), ply:Nick(), index, TTTLogTranslate(ply.DMGLogLang, "RDMWaiting"))
        local syncEnt = dlogs:GetSyncEnt()
    ...
    elseif status == RDM_MANAGER_FINISHED then
        msg = string.format(TTTLogTranslate(ply.DMGLogLang, "HasSetReport"), ply:Nick(), index, TTTLogTranslate(ply.DMGLogLang, "Finished"))
        local syncEnt = dlogs:GetSyncEnt()
...
net.Receive("dlogs.SendAnswer", function(_, ply)
    ...
    text = string.gsub(string.gsub(text, "[^%g\128-\191\194-\197\208-\210 ]+", ""), "%s+", " ")
    tbl.response = text

    for k, v in ipairs(player.GetHumans()) do
        if v:CanUseRDMManager() then
            v:dlogs_Notify(DAMAGELOG_NOTIFY_INFO, string.format(TTTLogTranslate(v.DMGLogLang, "HasAnsweredReport"), (v:IsActive() and TTTLogTranslate(v.DMGLogLang, "TheReportedPlayer") or ply:Nick()), index), 5, "vote_yes.wav")
            v:UpdateReport(previous, index)
        end
    end
...
net.Receive("dlogs.GetForgive", function(_, ply)
        ...
        if v:CanUseRDMManager() then
            if forgive then
                if v:IsActive() then
                    ...
                    v:dlogs_Notify(DAMAGELOG_NOTIFY_INFO, string.format(TTTLogTranslate(v.DMGLogLang, "HasCanceledReport"), ply:Nick(), index), 5, "vote_yes.wav")
                end
            else
                if v:IsActive() then
                    v:dlogs_Notify(DAMAGELOG_NOTIFY_INFO, TTTLogTranslate(v.DMGLogLang, "NoMercy") .. " #" .. index .. " !", 5, "vote_yes.wav")
                else
                    v:dlogs_Notify(DAMAGELOG_NOTIFY_INFO, string.format(TTTLogTranslate(v.DMGLogLang, "DidNotForgive"), ply:Nick(), tbl.attacker_nick, index), 5, "vote_yes.wav")
                end
            end
    ...
    if forgive then
        ply:dlogs_Notify(DAMAGELOG_NOTIFY_INFO, string.format(TTTLogTranslate(ply.DMGLogLang, "YouDecidedForgive"), tbl.attacker_nick), 5, "vote_yes.wav")
    else
        ply:dlogs_Notify(DAMAGELOG_NOTIFY_INFO, string.format(TTTLogTranslate(ply.DMGLogLang, "YouDecidedNotForgive"), tbl.attacker_nick), 5, "vote_no.wav")
    end
    ...
    if IsValid(attacker) then
        if forgive then
            attacker:dlogs_Notify(DAMAGELOG_NOTIFY_INFO, string.format(TTTLogTranslate(attacker.DMGLogLang, "DecidedToForgiveYou"), ply:Nick()), 5, "vote_yes.wav")
        else
            attacker:dlogs_Notify(DAMAGELOG_NOTIFY_INFO, string.format(TTTLogTranslate(attacker.DMGLogLang, "DecidedNotToForgiveYou"), ply:Nick()), 5, "vote_no.wav")
        end
    end
