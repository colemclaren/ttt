...
local function TakeAction()
	...
	//if ulx then
			...
			local function AddSlayPlayer(reported)
				...
				for k,v in ipairs( {"bullet_green.png", "bullet_yellow.png", "bullet_red.png"} )do
					...
					numbers:AddOption("Default reason", function()
						...
						if IsValid(ply) then
							RunConsoleCommand("mga", "aslay", ply:SteamID(), tostring(k))
							SetConclusion(ply:Nick(), k, "the default reason")
						else
							RunConsoleCommand("mga", "aslayid", (reported and report.attacker) or (not reported and report.victim), tostring(k))
					...
					numbers:AddOption("Set reason...", function()
						...
						Derma_StringRequest("Reason", "Please type the reason why you want to slay "..nick, "", function(txt)
							local ply = (reported and attacker) or (not reported and victim)
							if IsValid(ply) then
								RunConsoleCommand("mga", "aslay", ply:SteamID(), tostring(k), txt)
								SetConclusion(ply:Nick(), k, "\""..txt.."\"")
							else
								RunConsoleCommand("mga", "aslayid", (reported and report.attacker) or (not reported and report.victim), tostring(k), txt)
		...
		slaynr:AddOption("The reported player", function()
			if IsValid(attacker) then
				RunConsoleCommand("mga", "aslay", attacker:SteamID(), "0")
			else
				RunConsoleCommand("mga", "aslayid", report.attacker, "0")
			end
		end):SetImage("icon16/user_delete.png")
		slaynr:AddOption("The victim", function()
			if IsValid(victim) then
				RunConsoleCommand("mga", "aslay", victim:SteamID(), "0")
			else
				RunConsoleCommand("mga", "aslayid", report.victim, "0")
			end
