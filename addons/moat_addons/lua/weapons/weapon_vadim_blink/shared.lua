        ...
        if SERVER and IsValid( ply ) then
            ...
            if IsValid( rgd ) then
                ...
                for i = 0, rgd:GetPhysicsObjectCount() - 1 do
                    ...
                    timer.Simple( 0.1, function() phys:Sleep() end )
...
function SWEP:Uncloak( ply )
    ...
    timer.Simple( 0.75, function() if IsValid( ply ) then ply.BlinkImmunity = nil end end )
...
function SWEP:Deploy()
    ...
    if IsValid( self.Last ) then self:Uncloak( self.Last ) end
...
function SWEP:Trace()
    ...
    if not tr1.Hit then -- go look for a ledge ya bugger
        ...
        if tr2.Hit then final = tr2.HitPos + Vector( 0, 0, 8 ) end
...
function SWEP:Think()
    ...
    if self.Last ~= self.Owner then self.Last = self.Owner end
    ...
    if own:KeyDown( IN_ATTACK ) and self.Charge > 0 then
        ...
        if self.Active == -1 then
            ...
            if CLIENT then self.Ghost:SetSequence( own:GetSequence() ) end
    ...
    elseif self.Active > 0 then
        ...
        if timer.Exists( label ) then timer.Destroy( label ) end



        timer.Create( label, 0.02, 0, function()

            if not IsValid( self ) then timer.Destroy( label ) return end
        ...
        if SERVER then
            ...
            if self.FOV then own:SetFOV( self.FOV , 0 ) end
            ...
            if IsValid( vm ) then vm:SendViewModelMatchingSequence( 12 ) end
    ...
    if self.BlinkInfo then
        ...
        if CurTime() >= self.BlinkInfo[ 2 ] then self:Uncloak( own ) self.BlinkInfo = false return end
