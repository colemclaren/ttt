...
SWEP.CanBuy = {ROLE_DETECTIVE,TRAITOR}
...
function SWEP:Deploy()
  ...
  if (self.Owner:IsValid()) then self.Owner:SetJumpPower(600) end
...
function SWEP:Holster()
  ...
  if (self.Owner:IsValid()) then self.Owner:SetJumpPower(160) end
...
if SERVER then
  ...
  function GAMEMODE:OnPlayerHitGround(ply, in_water, on_floater, speed)
    ...
    if ply:GetActiveWeapon():GetClass() == "weapon_ttt_hjump" then damage = damage/70 end
    ...
    if math.floor(damage) > 0 then
      ...
      if IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "weapon_ttt_hjump" then
...
local function ResetJumpDeath( ply )
  ...
  if (ply:IsValid()) then ply:SetJumpPower(160) end
