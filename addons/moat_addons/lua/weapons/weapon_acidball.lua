...
function SWEP:Think()
	...
	if self.Owner:KeyDown(IN_ATTACK) and self.time >=50 then 
		if not self.Sound2 then
			self.Sound2 = CreateSound(self.Owner, "ambient/water/leak_1.wav")
			self.Sound2:Play()
		end
	else
		if self.Sound2 then self.Sound2:Stop() self.Sound2 = nil end
	...
	if not self.Owner:Alive() then if self.Sound2 then self.Sound2:Stop() self.Sound2 = nil end end
    ...
    if self.Owner:KeyDown(IN_ATTACK) then
		...
		if not self.Sound then
			self.Sound = CreateSound(self.Owner, "ambient/water/water_flow_loop1.wav")
			self.Sound:Play()
		end
	else
		if self.Sound then self.Sound:Stop() self.Sound = nil end
	...
	if not self.Owner:Alive() then if self.Sound then self.Sound:Stop() self.Sound = nil end end
    ...
    if CLIENT then
        ...
        self.VElements["acid"].size.y = self.ran1
		self.VElements["acid"].size = Vector(math.Rand(self.ach,self.ach+0.1), self.ran1, 0)
    end
end
...
function SWEP:Bong()
	...
	self.Weapon:EmitSound("ambient/water/water_splash"..math.random(1,3)..".wav", 400)
	...
	local ent = ents.Create( "acidball_ent" )
	...
	cleanup.Add( self.Owner, "acidball", ent )
