-- Generated from: glib/lua/glib/lua/decompiler/instruction.lua
-- Original:       https://github.com/notcake/glib/blob/master/lua/glib/lua/decompiler/instruction.lua
-- Timestamp:      2015-08-02 20:22:05
local self = {}
CAC.Lua.Instruction = CAC.MakeConstructor (self)
...
function self:SetOpcode (opcode)
	self.Opcode = opcode
	self.OpcodeName = CAC.Lua.Opcode [self.Opcode]
	self.OpcodeInfo = CAC.Lua.Opcodes:GetOpcode (self.Opcode)
end
...
function self:ToString ()
	...
	if self:GetOperandDType () == CAC.Lua.OperandType.None then
		-- A, B, C
...
function self:GetOperandValue (operand, operandType)
	if operandType == CAC.Lua.OperandType.Literal then
		return operand
	elseif operandType == CAC.Lua.OperandType.SignedLiteral then
		return operand < 32768 and operand or (operand - 65536)
	elseif operandType == CAC.Lua.OperandType.Primitive then
	...
	elseif operandType == CAC.Lua.OperandType.NumericConstantId then
	...
	elseif operandType == CAC.Lua.OperandType.StringConstantId then
	...
	elseif operandType == CAC.Lua.OperandType.RelativeJump then
...
function self:FormatOperand (operand, operandType)
	if operandType == CAC.Lua.OperandType.Variable then
	...
	elseif operandType == CAC.Lua.OperandType.DestinationVariable then
		local variableName = self.BytecodeReader:GetFrameVariableName (operand + 1)
		if variableName then return variableName end
		return "_" .. tostring (operand)
	elseif operandType == CAC.Lua.OperandType.UpvalueId then
		local upvalueName = self.BytecodeReader:GetUpvalueName (operand + 1)
		return upvalueName or ("_up" .. tostring (operand))
	elseif operandType == CAC.Lua.OperandType.Literal then
	...
	elseif operandType == CAC.Lua.OperandType.StringConstantId then
		local constantValue = self.BytecodeReader:GetGarbageCollectedConstantValue (self.BytecodeReader:GetGarbageCollectedConstantCount () - operand)
		if constantValue then
			return "\"" .. CAC.String.EscapeNonprintable (constantValue) .. "\""
		end
		return "str-" .. tostring (operand)
	elseif operandType == CAC.Lua.OperandType.RelativeJump then
		return tostring (operand - 0x8000)
	else
		return CAC.Lua.OperandType [operandType] .. " " .. tostring (operand)
	end
