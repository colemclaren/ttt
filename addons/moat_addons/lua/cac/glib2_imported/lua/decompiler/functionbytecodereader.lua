-- Generated from: glib/lua/glib/lua/decompiler/functionbytecodereader.lua
-- Original:       https://github.com/notcake/glib/blob/master/lua/glib/lua/decompiler/functionbytecodereader.lua
-- Timestamp:      2015-08-02 20:22:05
local self = {}
CAC.Lua.FunctionBytecodeReader = CAC.MakeConstructor (self)
...
function self:ctor (bytecodeReader, functionDump)
	...
	self.FrameSize      = 0
	self.FrameVariables = {}
	self.VariadicFrameVariable = CAC.Lua.FrameVariable (self, "...")
	...
	-- Read
	local reader = CAC.StringInBuffer (functionDump)
	...
	for i = 1, self.FrameSize do
		local frameVariable = CAC.Lua.FrameVariable (self, i)
	...
	for i = 1, self.GarbageCollectedConstantCount do
		...
		if constantType == CAC.Lua.GarbageCollectedConstantType.Function then
			-- Function
			constant = CAC.Lua.FunctionConstant ()
		elseif constantType == CAC.Lua.GarbageCollectedConstantType.Table then
			-- Table
			constant = CAC.Lua.TableConstant ()
		elseif constantType == CAC.Lua.GarbageCollectedConstantType.Int64 then
			-- Int64
			constant = CAC.Lua.GarbageCollectedConstant ()
			constant:SetType (constantType)
			CAC.Error ("Unhandled garbage collected constant type (int64)")
		elseif constantType == CAC.Lua.GarbageCollectedConstantType.UInt64 then
			-- UInt64
			constant = CAC.Lua.GarbageCollectedConstant ()
			constant:SetType (constantType)
			CAC.Error ("Unhandled garbage collected constant type (uint64)")
		elseif constantType == CAC.Lua.GarbageCollectedConstantType.Complex then
			-- Complex
			constant = CAC.Lua.GarbageCollectedConstant ()
			constant:SetType (constantType)
			CAC.Error ("Unhandled garbage collected constant type (complex)")
		elseif constantType >= CAC.Lua.GarbageCollectedConstantType.String then
			constant = CAC.Lua.StringConstant ()
		end
		constant:SetIndex (i)
	...
	for i = 1, self.NumericConstantCount do
		...
		if (low32 % 2) == 1 then
			...
			constant.Value = CAC.BitConverter.UInt32sToDouble (low32, high32)
		else
			low32 = math.floor (low32 / 2)
	...
	if not self.BytecodeReader:IsDebugInformationStripped () then
		...
		local debugReader = CAC.StringInBuffer (self.DebugData)
		...
		for i = 1, self.UpvalueCount do
			...
			if self.UpvalueNames [i] == "" or not CAC.Lua.IsValidVariableName (self.UpvalueNames [i]) then
...
function self:GetGarbageCollectedConstantEnumerator ()
	return CAC.ArrayEnumerator (self.GarbageCollectedConstants)
end
...
function self:GetInstruction (instructionId, instruction)
	instruction = instruction or CAC.Lua.Instruction (self)
...
function self:GetInstructionEnumerator ()
	local i = 0
	local instruction = CAC.Lua.Instruction (self)
	return function ()
		i = i + 1
...
function self:GetFrameVariableEnumerator ()
	return CAC.ArrayEnumerator (self.FrameVariables)
end
...
function self:ToString ()
	...
	local str = CAC.StringBuilder ()
	
	str:Append ("function (")
	local parameterVariable = CAC.Lua.FrameVariable (self)
...
function self:AnalyseJumps ()
	local targetInstruction = CAC.Lua.Instruction (self)
	...
	for instruction in self:GetInstructionEnumerator () do
		...
		if opcode == CAC.Lua.Opcode.JMP then
...
function self:DecompilePass1 ()
	...
	for instruction in self:GetInstructionEnumerator () do
		...
		assignmentExpressionPrecedence = CAC.Lua.Precedence.Lowest
		assignmentExpressionRawValue = nil
		destinationVariable = nil
		
		if instruction:GetOperandAType () == CAC.Lua.OperandType.DestinationVariable then
		...
		if opcodeName == "KSTR" then
			...
			assignmentExpressionPrecedence = CAC.Lua.Precedence.Atom
		...
		elseif opcodeName == "KNIL" then
			for i = instruction:GetOperandA (), instruction:GetOperandD () do
				...
				destinationVariable:AddStore (instruction:GetIndex (), "nil", CAC.Lua.Precedence.Atom, nil)
			end
		...
		-- Generic loads
		if instruction:GetOperandAType () == CAC.Lua.OperandType.Variable then
			variable = self:GetFrameVariable (instruction:GetOperandA () + 1)
			variable:AddLoad (instruction:GetIndex ())
		end
		if instruction:GetOperandBType () == CAC.Lua.OperandType.Variable then
			variable = self:GetFrameVariable (instruction:GetOperandB () + 1)
			variable:AddLoad (instruction:GetIndex ())
		end
		if instruction:GetOperandCType () == CAC.Lua.OperandType.Variable then
			variable = self:GetFrameVariable (instruction:GetOperandC () + 1)
			variable:AddLoad (instruction:GetIndex ())
		end
		if instruction:GetOperandDType () == CAC.Lua.OperandType.Variable then
...
function self:AnalyseFrameVariableUsage (frameVariable)
	local loadStore = CAC.Lua.LoadStore (frameVariable)
	local store = nil
	...
	while loadStore do
		if loadStore:IsStore () then
			...
			store = loadStore:Clone (store or CAC.Lua.LoadStore (frameVariable))
			loadCount = 0
		else
...
local function GetNextLoad (loadCache, frameVariable, instructionId)
	...
	loadStore = loadStore or CAC.Lua.LoadStore (frameVariable)
...
local conditionalOpcodes =
{
	...
	[CAC.Lua.Opcode.ISLE]  = ">",
	[CAC.Lua.Opcode.ISGT]  = "<=",
	[CAC.Lua.Opcode.ISEQV] = "~=",
	[CAC.Lua.Opcode.ISNEV] = "==",
	[CAC.Lua.Opcode.ISEQS] = "~=",
	[CAC.Lua.Opcode.ISNES] = "==",
	[CAC.Lua.Opcode.ISEQN] = "~=",
	[CAC.Lua.Opcode.ISNEN] = "==",
	[CAC.Lua.Opcode.ISEQP] = "~=",
	[CAC.Lua.Opcode.ISNEP] = "==",
}

local binaryOpcodes =
{
	[CAC.Lua.Opcode.ADDVN] = "+",
	[CAC.Lua.Opcode.SUBVN] = "-",
	[CAC.Lua.Opcode.MULVN] = "*",
	[CAC.Lua.Opcode.DIVVN] = "/",
	[CAC.Lua.Opcode.MODVN] = "%",
	[CAC.Lua.Opcode.ADDNV] = "+",
	[CAC.Lua.Opcode.SUBNV] = "-",
	[CAC.Lua.Opcode.MULNV] = "*",
	[CAC.Lua.Opcode.DIVNV] = "/",
	[CAC.Lua.Opcode.MODNV] = "%",
	[CAC.Lua.Opcode.ADDVV] = "+",
	[CAC.Lua.Opcode.SUBVV] = "-",
	[CAC.Lua.Opcode.MULVV] = "*",
	[CAC.Lua.Opcode.DIVVV] = "/",
	[CAC.Lua.Opcode.MODVV] = "%"
}

local binaryOpcodePrecedences =
{
	[CAC.Lua.Opcode.ADDVN] = CAC.Lua.Precedence.Addition,
	[CAC.Lua.Opcode.SUBVN] = CAC.Lua.Precedence.Subtraction,
	[CAC.Lua.Opcode.MULVN] = CAC.Lua.Precedence.Multiplication,
	[CAC.Lua.Opcode.DIVVN] = CAC.Lua.Precedence.Division,
	[CAC.Lua.Opcode.MODVN] = CAC.Lua.Precedence.Modulo,
	[CAC.Lua.Opcode.ADDNV] = CAC.Lua.Precedence.Addition,
	[CAC.Lua.Opcode.SUBNV] = CAC.Lua.Precedence.Subtraction,
	[CAC.Lua.Opcode.MULNV] = CAC.Lua.Precedence.Multiplication,
	[CAC.Lua.Opcode.DIVNV] = CAC.Lua.Precedence.Division,
	[CAC.Lua.Opcode.MODNV] = CAC.Lua.Precedence.Modulo,
	[CAC.Lua.Opcode.ADDVV] = CAC.Lua.Precedence.Addition,
	[CAC.Lua.Opcode.SUBVV] = CAC.Lua.Precedence.Subtraction,
	[CAC.Lua.Opcode.MULVV] = CAC.Lua.Precedence.Multiplication,
	[CAC.Lua.Opcode.DIVVV] = CAC.Lua.Precedence.Division,
	[CAC.Lua.Opcode.MODVV] = CAC.Lua.Precedence.Modulo
}
...
function self:DecompilePass3 ()
	...
	local loadStore = CAC.Lua.LoadStore ()
	...
	for instruction in self:GetInstructionEnumerator () do
		...
		elseif opcodeName == "TGETV" or
			...
			if cValue then
				if type (cValue) == "string" then
					if CAC.Lua.IsValidVariableName (cValue) then
						cExpression = cValue
						squareBrackets = false
					else
						cExpression = "\"" .. CAC.String.EscapeNonprintable (cValue) .. "\""
					end
				else
			...
			store:SetExpression (assignmentExpression, CAC.Lua.Precedence.Atom)
		...
		if conditionalOpcodes [opcode] then
			...
			if instruction:GetOperandAType () == CAC.Lua.OperandType.Variable then
				load = GetNextLoad (loadCache, aVariable, instruction:GetIndex ())
				aExpression = load:GetBracketedExpression (CAC.Lua.Precedence.Lowest)
			elseif instruction:GetOperandAType () == CAC.Lua.OperandType.StringConstantId then
				aExpression = "\"" .. CAC.String.EscapeNonprintable (instruction:GetOperandAValue ()) .. "\""
			else
			...
			if instruction:GetOperandDType () == CAC.Lua.OperandType.Variable then
				...
				dExpression = load:GetBracketedExpression (CAC.Lua.Precedence.Lowest)
			elseif instruction:GetOperandDType () == CAC.Lua.OperandType.StringConstantId then
				dExpression = "\"" .. CAC.String.EscapeNonprintable (instruction:GetOperandDValue ()) .. "\""
			else
		...
		if opcodeName == "IST" then
			load = GetNextLoad (loadCache, dVariable, instruction:GetIndex ())
			instruction:SetTag ("Lua", "if not " .. load:GetBracketedExpression (CAC.Lua.Precedence.LeftUnaryOperator) .. " then")
		...
		elseif opcodeName == "NOT" then
			...
			load = GetNextLoad (loadCache, dVariable, instruction:GetIndex ())
			assignmentExpression = "not " .. load:GetBracketedExpression (CAC.Lua.Precedence.LeftUnaryOperator)
			store:SetExpression (assignmentExpression, CAC.Lua.Precedence.LeftUnaryOperator)
		elseif opcodeName == "UNM" then
			isAssignment = true
			
			load = GetNextLoad (loadCache, dVariable, instruction:GetIndex ())
			assignmentExpression = "-" .. load:GetBracketedExpression (CAC.Lua.Precedence.LeftUnaryOperator)
			store:SetExpression (assignmentExpression, CAC.Lua.Precedence.LeftUnaryOperator)
		...
		if binaryOpcodes [opcode] then
			...
			elseif instruction:GetOperandBType () == CAC.Lua.OperandType.StringConstantId then
				bExpression = "\"" .. CAC.String.EscapeNonprintable (instruction:GetOperandBValue ()) .. "\""
			else
			...
			elseif instruction:GetOperandCType () == CAC.Lua.OperandType.StringConstantId then
				cExpression = "\"" .. CAC.String.EscapeNonprintable (instruction:GetOperandCValue ()) .. "\""
			else
		...
		elseif opcodeName == "CAT" then
			...
			for i = instruction:GetOperandB (), instruction:GetOperandC () do
				...
				assignmentExpression = assignmentExpression .. load:GetBracketedExpression (CAC.Lua.Precedence.Lowest)
			end
			store:SetExpression (assignmentExpression, CAC.Lua.Precedence.Lowest)
		end
