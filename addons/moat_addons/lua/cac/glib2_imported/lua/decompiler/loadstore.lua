-- Generated from: glib/lua/glib/lua/decompiler/loadstore.lua
-- Original:       https://github.com/notcake/glib/blob/master/lua/glib/lua/decompiler/loadstore.lua
-- Timestamp:      2015-08-02 20:22:05
local self = {}
CAC.Lua.LoadStore = CAC.MakeConstructor (self)
...
function self:GetBracketedExpression (outerPrecedence)
	...
	if outerPrecedence > precedence or
	   (outerPrecedence == precedence and not CAC.Lua.IsPrecedenceAssociative (precedence)) then
...
function self:GetExpression ()
	if self:IsLoad () then
		...
		if not expression then
			...
			expressionPrecedence = CAC.Lua.Precedence.Atom
		end
		expressionPrecedence = expressionPrecedence or CAC.Lua.Precedence.Lowest
