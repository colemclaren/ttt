-- Generated from: glib/lua/glib/lua/decompiler/bytecodereader.lua
-- Original:       https://github.com/notcake/glib/blob/master/lua/glib/lua/decompiler/bytecodereader.lua
-- Timestamp:      2015-08-02 20:22:05
local self = {}
CAC.Lua.BytecodeReader = CAC.MakeConstructor (self)
...
function self:ctor (functionOrDump, authId)
	...
	-- Read
	local reader = CAC.StringInBuffer (self.Dump)
	...
	if bit.band (self.Flags, CAC.Lua.BytecodeFlags.DebugInformationStripped) == 0 then
	...
	while functionDataLength ~= 0 do
		...
		self.Functions [#self.Functions + 1] = CAC.Lua.FunctionBytecodeReader (self, functionData)
...
function self:IsDebugInformationStripped ()
	return bit.band (self.Flags, CAC.Lua.BytecodeFlags.DebugInformationStripped) ~= 0
end
...
function self:ToString ()
	if not self.String then
		...
		if source then
			str:Append ("-- " .. CAC.String.EscapeNonprintable (source))
			str:Append ("\n")
		end
...
function self:LinkFunctions (lastUnallocatedFunction, functionBytecodeReader)
	for i = 1, functionBytecodeReader:GetGarbageCollectedConstantCount () do
		...
		if garbageCollectedConstant:GetType () == CAC.Lua.GarbageCollectedConstantType.Function then
