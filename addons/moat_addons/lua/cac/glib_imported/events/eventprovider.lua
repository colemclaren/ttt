-- Generated from: glib/lua/glib/events/eventprovider.lua
-- Original:       https://github.com/notcake/glib/blob/master/lua/glib/events/eventprovider.lua
-- Timestamp:      2016-02-22 19:22:23
local self = {}
CAC.EventProvider = CAC.MakeConstructor (self)
...
function self:DispatchEvent (eventName, ...)
	...
	if self.EventListeners [eventName] then
		for callbackName, callback in pairs (self.EventListeners [eventName]) do
			...
			local success, r0, r1, r2 = xpcall (callback, CAC.Error, ...)
	...
	elseif type (eventName) ~= "string" then
		CAC.Error ("EventProvider:DispatchEvent called incorrectly.")
	end
