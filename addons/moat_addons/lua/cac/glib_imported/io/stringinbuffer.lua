-- Generated from: glib/lua/glib/io/stringinbuffer.lua
-- Original:       https://github.com/notcake/glib/blob/master/lua/glib/io/stringinbuffer.lua
-- Timestamp:      2016-02-22 19:22:23
local self = {}
CAC.StringInBuffer = CAC.MakeConstructor (self, CAC.InBuffer)
...
function self:UInt64 ()
	...
	return CAC.BitConverter.UInt8sToUInt64 (uint80 or 0, uint81 or 0, uint82 or 0, uint83 or 0, uint84 or 0, uint85 or 0, uint86 or 0, uint87 or 0)
end

function self:Int8 ()
	local uint80 = string_byte (self.Data, self.Position, self.Position)
	self.Position = self.Position + 1
	return CAC.BitConverter.UInt8sToInt8 (uint80 or 0)
end

function self:Int16 ()
	local uint80, uint81 = string_byte (self.Data, self.Position, self.Position + 1)
	self.Position = self.Position + 2
	return CAC.BitConverter.UInt8sToInt16 (uint80 or 0, uint81 or 0)
end

function self:Int32 ()
	local uint80, uint81, uint82, uint83 = string_byte (self.Data, self.Position, self.Position + 3)
	self.Position = self.Position + 4
	return CAC.BitConverter.UInt8sToInt32 (uint80 or 0, uint81 or 0, uint82 or 0, uint83 or 0)
end

function self:Int64 ()
	local uint80, uint81, uint82, uint83, uint84, uint85, uint86, uint87 = string_byte (self.Data, self.Position, self.Position + 7)
	self.Position = self.Position + 8
	return CAC.BitConverter.UInt8sToInt64 (uint80 or 0, uint81 or 0, uint82 or 0, uint83 or 0, uint84 or 0, uint85 or 0, uint86 or 0, uint87 or 0)
end
