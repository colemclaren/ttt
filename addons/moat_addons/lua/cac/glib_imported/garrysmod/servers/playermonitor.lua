-- Generated from: glib/lua/glib/garrysmod/servers/playermonitor.lua
-- Original:       https://github.com/notcake/glib/blob/master/lua/glib/garrysmod/servers/playermonitor.lua
-- Timestamp:      2016-02-22 19:22:23
local self = {}
CAC.PlayerMonitor = CAC.MakeConstructor (self, CAC.IPlayerMonitor)
...
function self:ctor ()
	...
	self.DisconnectedPlayers = CAC.WeakKeyTable ()
	
	-- Players have to be queued because they might not have their steam IDs available yet.
	hook.Add (CLIENT and "OnEntityCreated" or "PlayerInitialSpawn", "CAC.PlayerMonitor.PlayerConnected",
		function (ply)
	...
	hook.Add ("Tick", "CAC.PlayerMonitor.ProcessQueue",
		function ()
			self:ProcessQueue ()
		end
	)
	...
	hook.Add ("player_disconnect", "CAC.PlayerMonitor.PlayerDisconnected",
		function (data)
...
function self:dtor ()
	hook.Remove (CLIENT and "OnEntityCreated" or "PlayerInitialSpawn", "CAC.PlayerMonitor.PlayerConnected")
...
function self:GetUserEntity (userId)
	...
	if not self.EntriesBySteamId [userId] then
		...
		for ply, _ in pairs (self.QueuedPlayers) do
			if CAC.GetPlayerId (ply) == userId then return ply end
		end
		return nil
	end
...
function self:GetUserEnumerator ()
	return CAC.KeyEnumerator (self.EntriesBySteamId)
end
...
function self:ProcessQueue ()
	...
	-- Process new players
	for ply, _ in pairs (self.QueuedPlayers) do
		local userId = CAC.GetPlayerId (ply)
		...
		elseif userId and
			...
			-- Add entry
			local entry = CAC.PlayerMonitorEntry (ply)
...
function self:__call (...)
	return CAC.PlayerMonitorProxy (self, ...)
end

CAC.PlayerMonitor = CAC.PlayerMonitor ()