-- Generated from: glib/lua/glib/bitconverter.lua
-- Original:       https://github.com/notcake/glib/blob/master/lua/glib/bitconverter.lua
...
-- Integers
function CAC.BitConverter.UInt8ToUInt8s (n)
	return n
end

function CAC.BitConverter.UInt16ToUInt8s (n)
	return             n        % 256,
	       math_floor (n / 256) % 256
end

function CAC.BitConverter.UInt32ToUInt8s (n)
	return             n             % 256,
	       math_floor (n /      256) % 256,
...
function CAC.BitConverter.UInt64ToUInt8s (n)
	return             n                      % 256,
	       math_floor (n /               256) % 256,
	       math_floor (n /             65536) % 256,
	       math_floor (n /          16777216) % 256,
...
function CAC.BitConverter.UInt8sToUInt8(uint80)
	return uint80
end

function CAC.BitConverter.UInt8sToUInt16 (uint80, uint81)
	return uint80 +
	       uint81 * 256
end

function CAC.BitConverter.UInt8sToUInt32 (uint80, uint81, uint82, uint83)
	return uint80 +
	       uint81 * 256 +
	       uint82 * 65536 +
	       uint83 * 16777216
...
function CAC.BitConverter.UInt8sToUInt64 (uint80, uint81, uint82, uint83, uint84, uint85, uint86, uint87)
	return uint80 +
	       uint81 * 256 +
	       uint82 * 65536 +
	       uint83 * 16777216 +
...
function CAC.BitConverter.Int8ToUInt8s (n)
	if n < 0 then n = n + 256 end
	return CAC.BitConverter.UInt8ToUInt8s (n)
end

function CAC.BitConverter.Int16ToUInt8s (n)
	if n < 0 then n = n + 65536 end
	return CAC.BitConverter.UInt16ToUInt8s (n)
end

function CAC.BitConverter.Int32ToUInt8s (n)
	if n < 0 then n = n + 4294967296 end
	return CAC.BitConverter.UInt32ToUInt8s (n)
end

function CAC.BitConverter.Int64ToUInt8s (n)
	local uint80, uint81, uint82, uint83 = CAC.BitConverter.UInt32ToUInt8s (n % 4294967296)
	local uint84, uint85, uint86, uint87 = CAC.BitConverter.Int32ToUInt8s (math_floor (n / 4294967296))
	return uint80, uint81, uint82, uint83, uint84, uint85, uint86, uint87
end

function CAC.BitConverter.UInt8sToInt8 (uint80)
	local n = CAC.BitConverter.UInt8sToUInt8 (uint80)
	if n >= 128 then n = n - 256 end
	return n
end

function CAC.BitConverter.UInt8sToInt16 (uint80, uint81)
	local n = CAC.BitConverter.UInt8sToUInt16 (uint80, uint81)
	if n >= 32768 then n = n - 65536 end
	return n
end

function CAC.BitConverter.UInt8sToInt32 (uint80, uint81, uint82, uint83)
	local n = CAC.BitConverter.UInt8sToUInt32 (uint80, uint81, uint82, uint83)
	if n >= 2147483648 then n = n - 4294967296 end
	return n
end

function CAC.BitConverter.UInt8sToInt64 (uint80, uint81, uint82, uint83, uint84, uint85, uint86, uint87)
	local low  = CAC.BitConverter.UInt8sToUInt32 (uint80, uint81, uint82, uint83)
	local high = CAC.BitConverter.UInt8sToInt32 (uint84, uint85, uint86, uint87)
	return low + high * 4294967296
end

-- IEEE floating point numbers
function CAC.BitConverter.FloatToUInt32 (f)
...
function CAC.BitConverter.DoubleToUInt32s (f)
	-- 1 / f is needed to check for -0
	local high = 0
	local low  = 0
	if f < 0 or 1 / f < 0 then
...
function CAC.BitConverter.UInt32ToFloat (n)
	-- 1 sign bit
...
function CAC.BitConverter.UInt32sToDouble (low, high)
	-- 1 sign bit
