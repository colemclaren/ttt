...
function ENT:Use(ply)
   ...
   if explode == 1 then
      ...
      if not self.exploding then
         ...
         timer.Simple(0.4, function() self:EmitSound("npc/attack_helicopter/aheli_damaged_alarm1.wav") self:SetColour(true) end)
...
function ENT:OnTakeDamage(dmginfo)
 ...
 --  if dmginfo:GetAttacker() == self:GetPlacer() then return end
   ...
   if IsPlayer(att) and dmglog == 1 then
      ...
      DamageLog(Format("%s damaged a fake portable tester for %d dmg",
