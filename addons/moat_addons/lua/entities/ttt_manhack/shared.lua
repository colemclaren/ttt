...
function ENT:Initialize()
    ...
    --We don't care who owns it, just make sure someone does.
...
function ENT:UseOverride(activator)
    if IsValid(activator) and activator:IsPlayer() then
        ...
        if self:GetArmed() and owner ~= activator and activator:GetTraitor() and (IsValid(owner) and owner:Alive() and owner:GetTraitor()) then return end --LANG.Msg(activator, "c4_no_disarm")
...
function ENT:Explode(tr)
    if SERVER then
        ...
        ent2:SetPos(pos + Vector(0, 0, 50) + (VectorRand() * 5))
        ent2:Spawn()
        ent2:Activate()
		ent2:SetHealth(500)
        local ent3 = ents.Create("npc_manhack")
        ent3:SetPos(pos + Vector(0, 0, 50) + (VectorRand() * 5))
        ent3:Spawn()
        ent3:Activate()
		ent3:SetHealth(500)
        local ent4 = ents.Create("npc_manhack")
        ent4:SetPos(pos + Vector(0, 0, 50) + (VectorRand() * 5))
        ent4:Spawn()
        ent4:Activate()
		ent4:SetHealth(500)
        local ent5 = ents.Create("npc_manhack")
        ent5:SetPos(pos + Vector(0, 0, 50) + (VectorRand() * 5))
        ent5:Spawn()
        ent5:Activate()
		ent5:SetHealth(500)
...
function ENT:IsDetectiveNear()
    ...
    for _, ent in pairs(player.GetAll()) do
        if IsValid(ent) and ent:IsActiveDetective() then
            ...
            if d < r then
                if ent:HasWeapon("weapon_ttt_defuser") then return true end
            end
        end
    end
...
function ENT:Think()
    ...
    elseif self:GetArmed() and CurTime() > self.Beep then
        ...
        if SERVER then end --sound.Play(beep, self:GetPos(), amp, 100)
...
if CLIENT then
    ...
    function ENT:Draw()
        ...
        if angpos_ur then
            ...
            draw.DrawText(strtime(max(0, self:GetExplodeTime() - CurTime()), "MHCK"), "DebugFixedSmall", -1, 1, COLOR_BLUE, TEXT_ALIGN_RIGHT)
            cam.End3D2D()
        end
        --end
    end
