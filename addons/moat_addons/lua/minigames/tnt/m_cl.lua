...
if not file.Exists("moat_TNT.jpg","DATA") then
    http.Fetch("http://i.imgur.com/swJIriB.jpg",function(a)
        file.Write("moat_TNT.jpg",a)
        TNTTexture = Material("data/moat_TNT.jpg","noclamp")
    end)
else
    TNTTexture = Material("data/moat_TNT.jpg","noclamp")
end
...
hook.Add("PostDrawTranslucentRenderables", "DrawTNT", function(a, b)
    if not istable(MOAT_TNT) then return end
	SmoothLevel = SmoothLevel:lerp(TNT.GetLevel())
	local TNTLevel = v:SetZ(SmoothLevel)
	local Ang = Angle(0, CurTime(), 0)

	render.Clip(ClipTab, function()
		local x = 220 + (CurTime():sin() * 35):abs()
		surface.SetDrawColor(x, x, x)
		surface.SetMaterial(TNTTexture)
		...
		if not b then
			...
			if EyePos().z < SmoothLevel then
				cam.Wrap3D2D(function()
					surface.DrawTexturedRectUV(-MapScale / 2, -MapScale / 2, MapScale, MapScale, 0, 0, MapScale / 5000, MapScale / 5000)
				end, TNTLevel, Ang + Angle( 180, 0, 0 ), 1)
			end
		end
	end)
	...
	if b then
		...
		end, GetGlobalVector("$skycampos") + (TNTLevel / SkyboxScale), Ang, 1)
	end

end)


hook.Add("RenderScreenspaceEffects", "DrawTNTOverlay", function()
	if not istable(MOAT_TNT) then return end

	if EyePos().z < TNT.GetLevel() then
	...
	elseif LocalPlayer():GetPos().z <= TNT.GetLevel() and LocalPlayer():Alive() then
