...
function MG_FFAS.StripWeapons(ply)
    for k, v in pairs(ply:GetWeapons()) do
        if (not MG_FFAS.DefaultLoadout[v:GetClass()]) then
            if (v.SetZoom) then
                v:SetZoom(false)
            end
...
function MG_FFAS.StartCommand(ply,cmd)
    ...
    if ply.SpawnProtection then
        if cmd:KeyDown(IN_ATTACK) then
            ...
            ply:SetNWInt("MG_FFAS_SPAWNPROTECTION",0)
        end
    end
end
...
function MG_FFAS.PlayerSpawn(ply)
    ...
    ply:SetNWInt("MG_FFAS_SPAWNPROTECTION",CurTime() + sp_time)
...
function MG_FFAS:PrepRound(mk, pri, sec, creds)
	...
	MG_FFAS.HookAdd("TTTBeginRound", MG_FFAS.BeginRound)
