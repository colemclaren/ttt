...
local textures 	= {}
local proxyurl 	= false --'https://YOUR_SITE.COM/?url=%s&width=%i&height=%i&format=%s'

if (not file.IsDir('moat/texture', 'DATA')) then
	file.CreateDir 'moat/texture'
else
	local files = file.Find('moat/texture/*', 'DATA')
	if (#files > 1000) then
		for k, v in ipairs(files) do
			file.Delete('moat/texture/' .. v)
		end
	end
end
...
function TEXTURE:Download(url, onsuccess, onfailure)
	...
	self.URL = url
	self.File = 'moat/texture/' .. self:GetUID() .. '.png'
	...
	if self.Cache and file.Exists(self.File, 'DATA') then
		...
		http.Fetch(self.Proxy and string.format(proxyurl, url:URLEncode(), self.Width, self.Height, self.Format) or url, function(body, len, headers, code)
			...
			local tempfile = 'moat/texture/tmp_' .. os.time() .. '_' .. self:GetUID() .. '.png'
...
function TEXTURE:RenderManual(func, callback)
	local cachefile = 'moat/texture/' .. self:GetUID() .. '-render.png'
		...
		hook.Add('PostRender', hookId, function()
			...
			render.PushRenderTarget(drawRT, 0, 0, w, h)
				...
				render.Clear(0, 0, 0, 0)
					...
					if self.Cache then
						self.File = 'moat/texture/' .. self:GetUID() .. '-render.png'
