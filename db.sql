...
CREATE TABLE IF NOT EXISTS `mg_itemstats` (
	...
	KEY `weaponid` (`weaponid`),
	CONSTRAINT `mg_itemstats` FOREIGN KEY (`weaponid`) REFERENCES `mg_items` (`id`) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS `mg_itemtalents` (
	...
	`updatedat` timestamp DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	KEY `weaponid` (`weaponid`),
	CONSTRAINT `mg_itemtalents` FOREIGN KEY (`weaponid`) REFERENCES `mg_items` (`id`) ON DELETE CASCADE
);
