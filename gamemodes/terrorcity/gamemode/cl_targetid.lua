...
function GetCurrentPlayerTrace()
    return hook.Run("TTTGetCurrentPlayerTrace")
end


function GM:TTTGetCurrentPlayerTrace()
    local p = LocalPlayer()
    ...
    return p, util.TraceLine {
        filter = filter,
        start = p:EyePos(),
        endpos = p:EyePos() + p:GetAimVector() * 10000,
        mask = MASK_SHOT,
    }
end

function GM:HUDDrawTargetID()
   local client, trace = GetCurrentPlayerTrace()
