...
local traitors = {[ROLE_TRAITOR] = true, [ROLE_HITMAN] = true}
function plymeta:GetTraitor()
    return traitors[self:GetRole()]
end

local dets = {[ROLE_DETECTIVE] = true}
function plymeta:GetDetective()
    return dets[self:GetRole()]
end
...
local killer = {[ROLE_KILLER] = true}
function plymeta:GetKiller()
...
function plymeta:IsActiveRole(role)
    return self:IsRole(role) and self:IsActive()
end
...
if CLIENT then
    ...
    function plymeta:HasWeapon(cls)
        for _, wep in pairs(self:GetWeapons()) do
            if IsValid(wep) and wep:GetClass() == cls then return true end
        end
...
function plymeta:GetEyeTrace(mask)
    if self.LastPlayerTraceMask == mask and self.LastPlayerTrace == CurTime() then return self.PlayerTrace end
...
if CLIENT then
    ...
    for _, a in pairs{ACT_GMOD_GESTURE_AGREE, ACT_GMOD_GESTURE_DISAGREE, ACT_GMOD_GESTURE_WAVE, ACT_GMOD_GESTURE_BECON, ACT_GMOD_GESTURE_BOW, ACT_GMOD_GESTURE_SALUTE, ACT_GMOD_CHEER, ACT_SIGNAL_FORWARD, ACT_SIGNAL_HALT, ACT_SIGNAL_GROUP, ACT_ITEM_PLACE, ACT_ITEM_DROP, ACT_ITEM_GIVE} do
