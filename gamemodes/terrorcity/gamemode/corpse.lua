...
local role_text_tbl = {
   ...
   [ROLE_KILLER]  = "body_found_k",
   [ROLE_DOCTOR]= "body_found_doc",
   [ROLE_BEACON] = "body_found_b",
   [ROLE_SURVIVOR]  = "body_found_s",
...
function CORPSE.ShowSearch(ply, rag, covert, long_range)
    ...
    if not hook.Run("TTTCanSearchCorpse", ply, rag, covert, long_range, (rag.was_role == ROLE_TRAITOR)) then return end
    ...
    if ply:CanUseCredits() and credits > 0 and (not long_range) then
        LANG.Msg(ply, "body_credits", {
            num = credits
        })
...
local function GetKillerSample(victim, attacker, dmg)
    ...
    if not (dmg:IsBulletDamage() or dmg:IsDamageType(DMG_SLASH) or dmg:IsDamageType(DMG_CLUB)) then return nil end
...
local poseparams = {"aim_yaw", "move_yaw", "aim_pitch"}
