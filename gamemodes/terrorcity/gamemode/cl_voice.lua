...
local function LastWordsRecv()
    ...
    chat.AddText(Color(150, 150, 150), Format("(%s) ", string.upper(GetTranslation("last_words"))), was_detective and Color(50, 200, 255) or Color(0, 200, 0), nick, COLOR_WHITE, ": " .. words)
end
...
local function RoleChatRecv()
    ...
    if role == ROLE_TRAITOR then
        chat.AddText(Color(255, 30, 40), Format("(%s) ", string.upper(GetTranslation("traitor"))), Color(255, 200, 20), sender:Nick(), Color(255, 255, 200), ": " .. text)
    elseif role == ROLE_DETECTIVE then
        chat.AddText(Color(20, 100, 255), Format("(%s) ", string.upper(GetTranslation("detective"))), Color(25, 200, 255), sender:Nick(), Color(200, 255, 255), ": " .. text)
    end
end
        ...
        if not radioframe then
            ...
            for key, command in pairs(self.Commands) do
                ...
                if command.format then
                    txt = txt .. GetPTranslation(command.text, {
                        player = GetTranslation("quick_nobody")
                    })
                else
                ...
                if command.format then
                    ...
                    dlabel.Think = function(s)
                        ...
                        if s.target ~= tgt then
                            ...
                            tgt = string.Interp(s.txt, {
                                player = RADIO.ToPrintable(tgt)
                            })
...
function RADIO.ToPrintable(target)
    ...
    elseif IsValid(target) then
        ...
        elseif target:GetClass() == "prop_ragdoll" then
            return GetPTranslation("quick_corpse_id", {
...
function RADIO:GetTarget()
    ...
    if IsValid(client) then
        ...
        if stored.target and stored.t > (CurTime() - 3) then return stored.target, stored.vague end
    end
...
local function RadioCommand(ply, cmd, arg)
    ...
    for _, msg in pairs(RADIO.Commands) do
        if msg.cmd == msg_type then
            ...
            text = msg.format and string.Interp(eng, {
                player = RADIO.ToPrintable(target)
            }) or eng
...
local function RadioMsgRecv()
    ...
    if lang_param then
        if lang_param == "quick_corpse_id" then
            ...
            param = GetPTranslation(lang_param, {
...
local function VoiceNotifyThink(pnl)
    ...
    if not (GetGlobal("ttt_locational_voice", false) and (not pnl.ply:IsSpec()) and (pnl.ply ~= LocalPlayer())) then return end
...
local function GetRechargeRate()
    local r = GetGlobal("ttt_voice_drain_recharge", 0.05)
...
local function GetDrainRate()
    if not GetGlobal("ttt_voice_drain", false) then return 0 end
    ...
    if ply:IsAdmin() or ply:IsDetective() then
        return GetGlobal("ttt_voice_drain_admin", 0)
    else
        return GetGlobal("ttt_voice_drain_normal", 0)
    end
end
...
function VOICE.Tick()
    if not GetGlobal("ttt_voice_drain", false) then return end
    local client = LocalPlayer()
...
function VOICE.CanSpeak()
    if not GetGlobal("ttt_voice_drain", false) then return true end
