...
local table = table
local pairs = pairs
local is_dmg = function(dmg_t, bit) return util.BitSet(tonumber(dmg_t), bit) end -- deal with large-number workaround for TableToJSON by -- parsing back to number here
    ...
    if table.Count(tr_killers) == 1 then
        ...
        if not table.HasValue(traitors, id) then
            ...
            return {
                ...
                text = T("aw_all1_text"),
                priority = math.random(0, table.Count(players))
            }
        end
    end
...
local function NumKills_Traitor(events, scores, players, traitors)
    ...
    if choices > 0 then
        ...
        elseif kills >= 4 and kills < 7 then
            return {
                title = T("aw_nkt4_title"),
                nick = nick,
                text = PT("aw_nkt4_text", {
                    num = kills
                }),
                priority = kills + 2
            }
...
local function CrowbarUser(events, scores, players, traitors)
    ...
    if kills > 1 and kills < 3 then
        ...
        award.text = PT("aw_cbr1_text", {
            num = kills
        })
    elseif kills >= 3 then
        award.title = T("aw_cbr2_title")

        award.text = PT("aw_cbr2_text", {
            num = kills
        })
...
local function SilencedPistolUser(events, scores, players, traitors)
    ...
    if kills > 1 and kills < 3 then
        ...
        award.text = PT("aw_sip1_text", {
            num = kills
        })
    elseif kills >= 3 then
        award.title = T("aw_sip2_title")

        award.text = PT("aw_sip2_text", {
            num = kills
        })
    else
        return nil
...
local function KnifeUser(events, scores, players, traitors)
    ...
    elseif kills > 1 and kills < 4 then
        ...
        award.text = PT("aw_knf3_text", {
            num = kills
        })
    elseif kills >= 4 then
        award.title = T("aw_knf4_title")

        award.text = PT("aw_knf4_text", {
            num = kills
        })
    else
        return nil
...
local function FlareUser(events, scores, players, traitors)
    ...
    if kills > 1 and kills < 3 then
        ...
        award.text = PT("aw_flg1_text", {
            num = kills
        })
    elseif kills >= 3 then
        award.title = T("aw_flg2_title")

        award.text = PT("aw_flg2_text", {
            num = kills
        })
    else
        return nil
...
local function M249User(events, scores, players, traitors)
    ...
    if kills > 1 and kills < 4 then
        ...
        award.text = PT("aw_hug1_text", {
            num = kills
        })
    elseif kills >= 4 then
        award.title = T("aw_hug2_title")

        award.text = PT("aw_hug2_text", {
            num = kills
        })
    else
        return nil
...
local function M16User(events, scores, players, traitors)
    ...
    if kills > 1 and kills < 4 then
        ...
        award.text = PT("aw_msx1_text", {
            num = kills
        })
    elseif kills >= 4 then
        award.title = T("aw_msx2_title")

        award.text = PT("aw_msx2_text", {
            num = kills
        })
    else
        return nil
