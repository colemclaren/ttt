...
local SpawnTypes = {"info_player_deathmatch", "info_player_combine", "info_player_rebel", "info_player_counterterrorist", "info_player_terrorist", "info_player_axis", "info_player_allies", "gmod_player_start", "info_player_teamspawn"}
...
local function PointsAroundSpawn(spwn)
    ...
    return {pos + Vector(w, 0, 0), pos + Vector(0, w, 0), pos + Vector(w, w, 0), pos + Vector(-w, 0, 0), pos + Vector(0, -w, 0), pos + Vector(-w, -w, 0), pos + Vector(-w, w, 0), pos + Vector(w, -w, 0)}
end
...
function GM:PlayerSelectSpawn(ply)
    ...
    for k, spwn in pairs(self.SpawnPoints) do
        if self:IsSpawnpointSuitable(ply, spwn, false) then return spwn end
    end
    ...
    for k, spwn in pairs(self.SpawnPoints) do
        if self:IsSpawnpointSuitable(ply, spwn, true) then return spwn end
    end
...
function GM:PlayerSpray(ply)
    if not IsValid(ply) or not ply:IsTerror() then return true end -- block
end
...
function GM:KeyPress(ply, key)
    ...
    if ply:IsSpec() and not ply:GetRagdollSpec() then
        if ply.propspec then return PROPSPEC.Key(ply, key) end
        ply:ResetViewRoll()
...
local function CheckCreditAward(victim, attacker)
    ...
    if (not victim:IsTraitor()) and (not GAMEMODE.AwardedCredits or GetConVar("ttt_credits_award_repeat"):GetBool()) then
        ...
        if pct >= GetConVarNumber("ttt_credits_award_pct") then
            ...
            if amt > 0 then
                LANG.Msg(GetTraitorFilter(true), "credit_tr_all", {
                    num = amt
                })
...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
    ...
    if ((not (ply.was_headshot or dmginfo:IsDamageType(DMG_SLASH) or (IsValid(killwep) and killwep.IsSilent))) and 
            (not (attacker.SilentPower and attacker.SilentPower > math.random() * 100))) then
        PlayDeathSound(ply)
    end
