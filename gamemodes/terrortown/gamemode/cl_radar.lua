...
function RADAR:Draw(client)
	...
	-- Radar blips are now toggleable
    ...
    for k, tgt in pairs(RADAR.targets) do
        ...
        if md < near_cursor_dist then
            alpha = math.Clamp(alpha * (md / near_cursor_dist), 40, 180)
        end
        ...
        elseif role == 3 then
            ...
            surface.SetTextColor(255, 0, 255, alpha)
        else
...
local function ReceiveCorpseCall()
    ...
    table.insert(RADAR.called_corpses, {
        pos = pos,
        called = CurTime()
    })
end
...
function RADAR.CreateMenu(parent, frame)
    ...
    dlabel:SetText(GetPTranslation("radar_help", {
        num = RADAR.duration
    }))
