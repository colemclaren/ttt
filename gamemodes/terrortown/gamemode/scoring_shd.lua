    ...
    if e.id == EVENT_KILL then
        ...
        elseif aid ~= -1 then
			...
			elseif e.vic.je then
				scores[aid].jester = scores[aid].jester + 1
...
function ScoreEventLog(events, scores, traitors, detectives, jester)
    for k, s in pairs(scores) do
		...
		scores[k].was_jester = table.HasValue(jester, k)
    end
...
function ScoreTeamBonus(scores, wintype)
    ...
    for k, sc in pairs(scores) do
        ...
        elseif (sc.was_jester) then
			state.jester = state.jester + 1
		else
	...
	if wintype == WIN_JESTER then
        bonus.jester = math.floor(alive.traitors * 1) + math.floor(alive.innos * 1) + math.ceil(dead.innos * 0.5)
    end
...
function KillsToPoints(score, was_traitor)
    --effectively 2 due to team bonus
    --for your own survival
    return ((score.suicides * -1) + score.bonus + (score.traitors * (was_traitor and -16 or 5)) + (score.innos * (was_traitor and 1 or -8)) + (score.deaths == 0 and 1 or 0))
end
