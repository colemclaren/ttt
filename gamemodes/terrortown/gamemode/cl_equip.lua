...
local function PreqLabels(parent, x, y)
    ...
    tbl.credits.Check = function(s, sel)
        ...
        return credits > 0, GetPTranslation("equip_cost", {
            num = credits
        })
    end
    ...
    tbl.owned.Check = function(s, sel)
        if ItemIsWeapon(sel) and (not CanCarryWeapon(sel)) then
            return false, GetPTranslation("equip_carry_slot", {
                slot = sel.slot
            })
        elseif (not ItemIsWeapon(sel)) and LocalPlayer():HasEquipmentItem(sel.id) then
...
local function TraitorMenuPopup()
    ...
    for k, item in pairs(items) do
        ...
        if ((not can_order) or table.HasValue(owned_ids, item.id) or (tonumber(item.id) and ply:HasEquipmentItem(tonumber(item.id))) or (ItemIsWeapon(item) and (not CanCarryWeapon(item))) or (item.limited and ply:HasBought(tostring(item.id)))) then
            -- already owned
