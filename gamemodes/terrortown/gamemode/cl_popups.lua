...
local function GetTextForRole(role)
    ...
    elseif role == ROLE_DETECTIVE then
        return GetPTranslation("info_popup_detective", {
        ...
        if #traitors > 1 then
            ...
            text = GetPTranslation("info_popup_traitor_alone", {
                menukey = menukey
            })
        end
...
local function IdlePopup()
    ...
    local idle_limit = GetGlobal("ttt_idle_limit") or 300
    local text = vgui.Create("DLabel", inner)
    text:SetWrap(true)

    text:SetText(GetPTranslation("idle_popup", {
        num = idle_limit,
        helpkey = Key("gm_showhelp", "F1")
    }))
