...
function GM:PlayerCanSeePlayersChat(text, team_only, listener, speaker)
    ...
    if (GetRoundState() ~= ROUND_ACTIVE) or (not GetConVar("ttt_limit_spectator_chat"):GetBool()) or (not DetectiveMode()) or (not sTeam and ((team_only and not speaker:IsSpecial()) or (not team_only))) or (not sTeam and team_only and speaker:GetRole() == listener:GetRole()) or (sTeam and lTeam) then return true end -- Round isn't active -- Spectators can chat freely -- Mumbling -- If someone alive talks (and not a special role in teamchat's case) -- If the speaker and listener are spectators
...
function GM:PlayerCanHearPlayersVoice(listener, speaker)
    -- Enforced silence
    if mute_all then return false, false end
    if (not IsValid(speaker)) or (not IsValid(listener)) or (listener == speaker) then return false, false end
    ...
    if speaker:IsSpec() and (not listener:IsSpec()) and limit and GetRoundState() == ROUND_ACTIVE then return false, false end
    -- Specific mute
    if listener:IsSpec() and listener.mute_team == speaker:Team() or listener.mute_team == MUTE_ALL then return false, false end
    -- Specs should not hear each other locationally
    if speaker:IsSpec() and listener:IsSpec() then return true, false end
...
local function RadioCommand(ply, cmd, args)
    if ply.CockBlockGameMsg then return end
    ...
    if IsValid(ply) and ply:IsTerror() and #args == 2 then
        ...
        net.Broadcast()
        ply.CockBlockGameMsg = true

        timer.Simple(0.1, function()
            ply.CockBlockGameMsg = false
        end)
    end
end
