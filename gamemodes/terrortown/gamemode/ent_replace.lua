...
local function ReplaceSingle(ent, newname)
    ...
    if ent:GetPos() == vector_origin then return end
...
local broken_parenting_ents = {"move_rope", "keyframe_rope", "info_target", "func_brush"}
...
local dummify = {"hostage_entity", "item_ammopack_full", "item_ammopack_medium", "item_ammopack_small", "item_healthkit_full", "item_healthkit_medium", "item_healthkit_small", "item_teamflag", "game_intro_viewpoint", "info_observer_point", "team_control_point", "team_control_point_master", "team_control_point_round", "item_ammo_revolver"} -- CS:S -- TF2 -- ZM
...
local function PlaceExtraWeaponsForCSS()
    ...
    local spots_classes = {"info_player_terrorist", "info_player_counterterrorist", "hostage_entity"}
...
local function PlaceExtraWeaponsForTF2()
    ...
    local spots_classes = {"info_player_teamspawn", "team_control_point", "team_control_point_master", "team_control_point_round", "item_ammopack_full", "item_ammopack_medium", "item_ammopack_small", "item_healthkit_full", "item_healthkit_medium", "item_healthkit_small", "item_teamflag", "game_intro_viewpoint", "info_observer_point"}
    PlaceWeaponsAtEnts(spots_classes)
end
...
function ents.TTT.PlaceExtraWeapons()
    ...
    for k, v in pairs(ents.FindByClass("weapon_*")) do
        ...
        if IsValid(v) and v.AutoSpawnable and not IsValid(v:GetOwner()) then return end
    end
