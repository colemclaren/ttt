...
function LANG.CreateLanguage(lang_name)
    ...
    if lang_name == LANG.DefaultLanguage then
        ...
        setmetatable(LANG.Strings[lang_name], {
            __index = function(tbl, name) return Format("[ERROR: Translation of %s not found]", name), false end
        })
    end
...
function GetRoleColor(role)
    return hook.Run("TTTGetRoleColor", role) or Color(25, 200, 25, 200)
end
...
function GM:TTTGetRoleColor(role)
    ...
    return bgcolor[role] or Color(25, 200, 25, 200)
end
...
LANG.Styles = {
    ...
    chat_warn = function(text)
        chat.AddText(Material("icon16/exclamation.png"), COLOR_RED, text)
    end,
    chat_plain = function(text)
        chat.AddText(Material("icon16/information.png"), Color(0, 128, 255), text)
    end
}
...
local styledmessages = {
    rolecolour = {"round_traitors_one", "round_traitors_more", "buy_no_stock", "buy_pending", "buy_received", "xfer_no_recip", "xfer_no_credits", "xfer_success", "xfer_received", "c4_no_disarm", "tele_failed", "tele_no_mark", "tele_marked", "dna_identify", "dna_notfound", "dna_limit", "dna_decayed", "dna_killer", "dna_no_killer", "dna_armed", "dna_object", "dna_gone", "credit_det_all", "credit_tr_all", "credit_kill"},
    chat_plain = {"body_call", "disg_turned_on", "disg_turned_off"},
    chat_warn = {"spec_mode_warning", "radar_not_owned", "radar_charging", "drop_no_room", "body_burning", "tele_no_ground", "tele_no_crouch", "tele_no_mark_ground", "tele_no_mark_crouch", "drop_no_ammo"}
}
